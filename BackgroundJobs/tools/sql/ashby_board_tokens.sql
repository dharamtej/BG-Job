-- Ashby Board Tokens — 2985 company slugs
-- API pattern : GET https://api.ashbyhq.com/posting-api/job-board/{slug}?includeCompensation=true
-- Board URL   : https://jobs.ashbyhq.com/{slug}
--
-- Usage: Run once in Railway PostgreSQL console.
-- Status starts as 'UNKNOWN' — the handler will mark invalid ones after first run.

CREATE TABLE IF NOT EXISTS api.ashby_board_tokens (
...
INSERT INTO api.ashby_board_tokens (company_name, board_token, api_url, board_url, status, created_at, updated_at)
SELECT
    INITCAP(REPLACE(slug, '-', ' ')) AS company_name,
    slug                             AS board_token,
    'https://api.ashbyhq.com/posting-api/job-board/' || slug || '?includeCompensation=true' AS api_url,
    'https://jobs.ashbyhq.com/' || slug AS board_url,
    'UNKNOWN'                        AS status,
    NOW()                            AS created_at,
    NOW()                            AS updated_at
FROM (VALUES
