-- Lever Board Tokens — 4,368 company slugs
-- Source: github.com/Feashliaa/job-board-aggregator (CC BY-NC 4.0)
-- API pattern : GET https://api.lever.co/v0/postings/{slug}?mode=json
-- Board URL   : https://jobs.lever.co/{slug}
--
-- Usage: Run once in Railway PostgreSQL console.
-- Status starts as 'UNKNOWN' — the handler will mark invalid ones after first run.

CREATE TABLE IF NOT EXISTS api.lever_board_tokens (
    id           SERIAL PRIMARY KEY,
    company_name VARCHAR(255) NOT NULL,
    board_token  VARCHAR(255) NOT NULL UNIQUE,
    industry     VARCHAR(255),
    status       VARCHAR(50)  NOT NULL DEFAULT 'UNKNOWN',
    http_code    SMALLINT,
    job_count    INTEGER,
    api_url      VARCHAR(500),
    board_url    VARCHAR(500),
    created_at   TIMESTAMP    NOT NULL DEFAULT NOW(),
    updated_at   TIMESTAMP    NOT NULL DEFAULT NOW()
);

INSERT INTO api.lever_board_tokens (company_name, board_token, api_url, board_url, status, created_at, updated_at)
SELECT
    INITCAP(REPLACE(slug, '-', ' ')) AS company_name,
    slug                             AS board_token,
    'https://api.lever.co/v0/postings/' || slug || '?mode=json' AS api_url,
    'https://jobs.lever.co/'            || slug                  AS board_url,
    'UNKNOWN'                        AS status,
    NOW()                            AS created_at,
    NOW()                            AS updated_at
FROM (VALUES
    ('10up-2'),
    ('15five'),
    ('174powerglobal'),
    ('1776'),
    ('18c'),
    ('1910genetics'),
    ('1inch'),
    ('1password'),
    ('21'),
    ('21hhs'),
    ('2a'),
    ('2brains'),
    ('360learning'),
    ('3box'),
    ('3dhubs'),
    ('3eco'),
    ('3pillarglobal'),
    ('3xmgroup'),
    ('42dot'),
    ('4front'),
    ('4mation'),
    ('4sightsupplychain'),
    ('500'),
    ('522productions'),
    ('54collective'),
    ('5amventures'),
    ('5wpr'),
    ('5xdata'),
    ('7geese'),
    ('8thlight'),
    ('8x8'),
    ('90seconds'),
    ('99taxis'),
    ('abacus'),
    ('ableserve'),
    ('ablspacesystems'),
    ('abovelending'),
    ('abridge'),
    ('abt'),
    ('abtasty'),
    ('academia'),
    ('academy'),
    ('acala'),
    ('acast'),
    ('acceldata'),
    ('accesso'),
    ('accesssoftek'),
    ('accompanyhealth'),
    ('accrue'),
    ('accurate'),
    ('acds'),
    ('achievers'),
    ('acl'),
    ('aclaimant'),
    ('acpl'),
    ('acselhealth'),
    ('act'),
    ('actian'),
    ('actionlife'),
    ('activecampaign'),
    ('actriv'),
    ('ad2games'),
    ('ada'),
    ('ada-diamonds'),
    ('adaptavist'),
    ('adcolony'),
    ('additionwealth'),
    ('addx'),
    ('adeptailabs'),
    ('adhoclabs'),
    ('adidevelopments'),
    ('adora'),
    ('adtech'),
    ('adthena'),
    ('advancedagrilytics'),
    ('advancednavigation'),
    ('adventus'),
    ('adverum'),
    ('advocate'),
    ('ae-2'),
    ('aechelon'),
    ('aeler'),
    ('aeratechnology'),
    ('aerialapplications'),
    ('aero'),
    ('aerostrat'),
    ('aethoshotels'),
    ('aeva'),
    ('afarmedia'),
    ('afero'),
    ('affinity'),
    ('affirm'),
    ('afreshtechnologies'),
    ('afterpaytouch'),
    ('agatesoftware'),
    ('agentnoon'),
    ('agerpoint'),
    ('agicap'),
    ('agile-defense'),
    ('agilesearch'),
    ('agiloft'),
    ('agotai'),
    ('agreena'),
    ('agtonomy'),
    ('ahschc'),
    ('aicamp'),
    ('aicrete'),
    ('aidaily'),
    ('aidash'),
    ('aifi'),
    ('aifoundation'),
    ('aifund'),
    ('aigen'),
    ('ailens'),
    ('aipconnect'),
    ('air-tek'),
    ('aira'),
    ('airalo'),
    ('aircall'),
    ('airdna'),
    ('airhouse'),
    ('airprotein'),
    ('airship'),
    ('airshipsyndicate'),
    ('airslate'),
    ('airtm'),
    ('airwallex'),
    ('aisafety'),
    ('aiwyn'),
    ('aizon'),
    ('ajax'),
    ('ajccanada'),
    ('akabrands'),
    ('akashnetwork'),
    ('akitasoftware'),
    ('alan'),
    ('alation'),
    ('albedo'),
    ('alchemy'),
    ('alcion'),
    ('aldar'),
    ('alector'),
    ('aledade'),
    ('aleph'),
    ('alertus'),
    ('aleysian'),
    ('algolia'),
    ('algorex'),
    ('algotive'),
    ('alice'),
    ('alice-bob'),
    ('alictus'),
    ('alida'),
    ('alifsemi'),
    ('alignable'),
    ('alimentiv-2'),
    ('alkymi'),
    ('allata'),
    ('allears-vet'),
    ('allegiantair'),
    ('alliance'),
    ('alliedreit'),
    ('allinbits'),
    ('alloy'),
    ('allstripes'),
    ('alltrails'),
    ('alluxio'),
    ('allworknow'),
    ('alpineimmunesciences'),
    ('alpinestars'),
    ('altaml'),
    ('altarum'),
    ('altoida'),
    ('altrio'),
    ('aluna'),
    ('alveotechnologies'),
    ('amanotes'),
    ('amaro'),
    ('amaze'),
    ('amberbio'),
    ('ambercycle'),
    ('amberelectric'),
    ('amberflo'),
    ('ambergroup'),
    ('ambirobotics'),
    ('ambrook'),
    ('americantall'),
    ('amerisave'),
    ('amiri'),
    ('amityfdn'),
    ('amobee'),
    ('ampd-energy'),
    ('ampeco-global'),
    ('amplitude'),
    ('ams-corp'),
    ('amtengineering'),
    ('amused-group'),
    ('amyris'),
    ('ana-corp'),
    ('anagram'),
    ('analog'),
    ('analogfolk'),
    ('analuisa'),
    ('analyticpartners'),
    ('anavationllc'),
    ('ancestry'),
    ('anchorage'),
    ('andersencorp'),
    ('andersgroup'),
    ('anduril'),
    ('angaza'),
    ('angel'),
    ('angellist'),
    ('anglehealth'),
    ('animaapp'),
    ('animaker'),
    ('animocabrands'),
    ('animoto'),
    ('anjuna'),
    ('anlatan'),
    ('annalise'),
    ('anomali'),
    ('anomalo'),
    ('anotherco'),
    ('anovium'),
    ('ansatzcapital'),
    ('ansira'),
    ('anthropic'),
    ('anticapital'),
    ('antisocialsocialcapital'),
    ('anybotics'),
    ('anyperk'),
    ('anyroad'),
    ('anyscale'),
    ('anysignal'),
    ('anzen'),
    ('aofl'),
    ('aon3d'),
    ('apartmentlife'),
    ('apartmentlist'),
    ('apax'),
    ('apdscorporate'),
    ('aperturehealth'),
    ('apex'),
    ('apex-ai'),
    ('apexpi'),
    ('api-ux'),
    ('apible'),
    ('apogeetherapeutics'),
    ('apollo'),
    ('apolloagriculture'),
    ('apollographql'),
    ('apolloresearch'),
    ('aposto'),
    ('appacademy'),
    ('appboxo'),
    ('appcues-2'),
    ('appen'),
    ('appen-2'),
    ('appfollow'),
    ('apphub'),
    ('appliedminds'),
    ('applike'),
    ('applovin'),
    ('applyboard'),
    ('applydigital'),
    ('appnation'),
    ('appsamurai'),
    ('appsilon'),
    ('apptegy'),
    ('apptimize'),
    ('appzen'),
    ('aprio'),
    ('apt-deco'),
    ('aptible'),
    ('aquabyte'),
    ('aquicore'),
    ('arable'),
    ('aragon'),
    ('arbitalhealth'),
    ('arbitrumfoundation'),
    ('arbol'),
    ('arbor'),
    ('arborenergy'),
    ('arcade'),
    ('arcadia'),
    ('arcadiascience'),
    ('arceoanalytics'),
    ('archive'),
    ('archivecom'),
    ('arcoeducacao'),
    ('area1security'),
    ('ariwalsh'),
    ('ark-solutions-inc'),
    ('arkbiotech'),
    ('arootah'),
    ('arpa'),
    ('arrive'),
    ('arrivelogistics'),
    ('arsenalbio'),
    ('arsiem'),
    ('artbio'),
    ('artblocks'),
    ('artera'),
    ('artera-2'),
    ('articulate'),
    ('artificial'),
    ('asap'),
    ('asapp-2'),
    ('ascoeq'),
    ('asfar'),
    ('ashoka'),
    ('asimov'),
    ('askfavor'),
    ('aspiration-2'),
    ('assurance'),
    ('astera'),
    ('asthomecares'),
    ('astra'),
    ('astranis'),
    ('ataccama'),
    ('atavistik-bio'),
    ('atekinc'),
    ('athennian'),
    ('athletetoathlete'),
    ('athleticgreens'),
    ('atlan'),
    ('atlassian'),
    ('atmosera'),
    ('atmosphere'),
    ('atob'),
    ('atomcomputing'),
    ('atomi'),
    ('atomic'),
    ('atomicmachines'),
    ('atonarp'),
    ('atrium'),
    ('attabotics'),
    ('attackiq'),
    ('attentive'),
    ('attentivemobile'),
    ('attn'),
    ('attovia-therapeutics'),
    ('audinate'),
    ('audius'),
    ('augmedix'),
    ('augustbioservices'),
    ('aurora-dev'),
    ('aurorainsight'),
    ('aurorasolar'),
    ('auth0'),
    ('authentic8'),
    ('autify'),
    ('autofi'),
    ('automatiq'),
    ('automox'),
    ('autonomous'),
    ('autonomys'),
    ('auw'),
    ('av'),
    ('ava'),
    ('avalabs'),
    ('avalanchestudios'),
    ('avalerehealth'),
    ('avante'),
    ('avantstay'),
    ('avela'),
    ('aventon'),
    ('avero'),
    ('avertium'),
    ('aviahealth'),
    ('avioconsulting'),
    ('avivesolutions'),
    ('aviyatech'),
    ('avratalent'),
    ('avtex'),
    ('awake-security'),
    ('axelera'),
    ('axiomzen'),
    ('axionray'),
    ('axon'),
    ('axoni'),
    ('axonify'),
    ('azavea'),
    ('aztal'),
    ('azul'),
    ('babylonhealth'),
    ('back'),
    ('backbone'),
    ('backflip'),
    ('backmarket'),
    ('backstage'),
    ('badal'),
    ('bai-inc'),
    ('balbix'),
    ('baller'),
    ('bambee'),
    ('bamko'),
    ('bamtradingservices'),
    ('banglalink'),
    ('bannerbank'),
    ('baoinc'),
    ('barbaricum'),
    ('barecremation'),
    ('barefootproximity'),
    ('barkleyus'),
    ('barley'),
    ('barrel'),
    ('base-flowa'),
    ('baselane'),
    ('basis'),
    ('batterymetals'),
    ('battlecreekchurch'),
    ('bayzat'),
    ('bazaarvoice'),
    ('bb2bank'),
    ('bcg'),
    ('bdg'),
    ('beacon'),
    ('beacondigitalmarketing'),
    ('beam'),
    ('beam-impact'),
    ('beamy'),
    ('beanstalkagtech'),
    ('beautybarrage'),
    ('beautyhaul'),
    ('beaverprocess'),
    ('bebop'),
    ('bedrockenergy'),
    ('bedrockocean'),
    ('bee-talents'),
    ('beedie'),
    ('beegreen'),
    ('beemenergy'),
    ('beetalents'),
    ('beghouconsulting'),
    ('bekk'),
    ('belkins'),
    ('bellese'),
    ('bellhops'),
    ('bellotalabs'),
    ('bellwetheram-2'),
    ('belong'),
    ('belonghome'),
    ('belvederetrading'),
    ('benchling'),
    ('benchsci'),
    ('benlabs'),
    ('bentoboxent'),
    ('bespokepost'),
    ('bestegg'),
    ('beta'),
    ('betr'),
    ('betstamp'),
    ('better'),
    ('betterlifepartners'),
    ('betterplaceforests'),
    ('beyond-creative'),
    ('beyondcloudconsulting'),
    ('beyondmeat'),
    ('beyondpricing'),
    ('bfp'),
    ('bfsaul'),
    ('bfsaulhotels'),
    ('bhg-inc'),
    ('bhhc'),
    ('bhvr'),
    ('biconomy'),
    ('bigblue'),
    ('bigfootbiomedical'),
    ('biggergames'),
    ('biggie'),
    ('bighealth'),
    ('bigtime'),
    ('bill'),
    ('bimone'),
    ('binance'),
    ('binti'),
    ('bioagilytix'),
    ('biobot'),
    ('biodigital'),
    ('bioenergydevco'),
    ('biointellisense'),
    ('biorender'),
    ('bird'),
    ('birdcanada'),
    ('bis'),
    ('bisnow'),
    ('bitdefender'),
    ('bitly'),
    ('bitnami'),
    ('bitpanda'),
    ('bitquilltech'),
    ('bitrise'),
    ('bitwiseinvestments'),
    ('bixal'),
    ('bizlibrary'),
    ('bkln'),
    ('blablacar'),
    ('black-white-zebra'),
    ('blackbirdinteractive'),
    ('blackcloak'),
    ('blackcrow'),
    ('blackgirls-code'),
    ('blackpanda'),
    ('blackriflecoffee'),
    ('blanc-labs'),
    ('blencorp'),
    ('blendlabs'),
    ('blinkux'),
    ('blinq'),
    ('blocknative'),
    ('blockspring'),
    ('blockstack'),
    ('blockswap'),
    ('blocpower'),
    ('bloom'),
    ('bloomcredit'),
    ('bloomon'),
    ('bloomthat'),
    ('bloomwellcare'),
    ('blox-route'),
    ('blue'),
    ('blueboard'),
    ('bluebottlecoffee'),
    ('bluecatnetworks'),
    ('bluecoding'),
    ('bluecore'),
    ('bluedot'),
    ('blueengine'),
    ('bluejlegal'),
    ('bluelabs'),
    ('blueland'),
    ('bluelightconsulting'),
    ('blueoceanai'),
    ('blueoceanbarns'),
    ('blueprinttitle'),
    ('bluerocktx'),
    ('bluesight'),
    ('bluesinc'),
    ('bluespace'),
    ('bluewheelmedia'),
    ('blum'),
    ('boastai'),
    ('boats'),
    ('bodo'),
    ('bodyport'),
    ('bogaardgroup'),
    ('boldbusiness'),
    ('bolster'),
    ('bolt'),
    ('boltbio'),
    ('boltjobs'),
    ('bombfell'),
    ('bonedry'),
    ('bonfirestudios'),
    ('bonial-2'),
    ('bonsaileadershipgroup'),
    ('bonusly'),
    ('bookeeapp'),
    ('booknooklearning'),
    ('boomandbucket'),
    ('boostedboards'),
    ('boringcompany'),
    ('bosonai'),
    ('bosta'),
    ('boston-materials'),
    ('bostondynamicsaiinstitute'),
    ('bostonimaging'),
    ('botify'),
    ('bottlerocketstudios'),
    ('boulderingproject'),
    ('boulevard'),
    ('bound'),
    ('bounteous'),
    ('boweryfarming'),
    ('box'),
    ('boxbot'),
    ('boxlunch'),
    ('bpmcpa'),
    ('brafton'),
    ('brainchildbio'),
    ('brainnest'),
    ('brainstorminc'),
    ('brand-knew'),
    ('braveventurelabs'),
    ('braviantholdings'),
    ('brazoderecho'),
    ('breakwatertech'),
    ('brevo'),
    ('brewbike'),
    ('brewbird'),
    ('bricknetworks'),
    ('bridge'),
    ('bridge2rwanda'),
    ('bridgebio'),
    ('brighte'),
    ('brightedge'),
    ('brighthealthplan'),
    ('brightmachines'),
    ('brightonjones'),
    ('brightseedbio'),
    ('brightspot'),
    ('brightwheel'),
    ('brilliant'),
    ('brillio-2'),
    ('brimstone'),
    ('brimstoneenergy'),
    ('brinc'),
    ('brindleyengineering'),
    ('britecore'),
    ('broadwaytechnology'),
    ('brooksrunning'),
    ('brotherhoodcrusade'),
    ('bssp'),
    ('btc'),
    ('btse'),
    ('buckmason'),
    ('buildatmos'),
    ('buildgc'),
    ('buildkite'),
    ('buildzoom'),
    ('builtrobotics'),
    ('bukuwarung'),
    ('bumbleinc'),
    ('bunches'),
    ('bundle'),
    ('bungalow'),
    ('buoyhealth'),
    ('buoysoftware'),
    ('burga'),
    ('burnt'),
    ('burro'),
    ('busaracenter'),
    ('businesswire'),
    ('bustle'),
    ('butcherbox'),
    ('butterpayments'),
    ('button'),
    ('buyboxexperts'),
    ('bv'),
    ('bvaccel'),
    ('c12qe'),
    ('c16bio'),
    ('cabonorte'),
    ('cagents'),
    ('caliza-financial-technologies'),
    ('call9'),
    ('callan'),
    ('callistomedia'),
    ('calm'),
    ('calstart'),
    ('calven'),
    ('camber'),
    ('cambiumnetworks'),
    ('cambly'),
    ('campspot'),
    ('campusworksinc'),
    ('canarytechnologies'),
    ('candlinspection'),
    ('canimmunize'),
    ('canopyservicing'),
    ('canva'),
    ('canvasmedical'),
    ('canvasww'),
    ('capchase'),
    ('capital'),
    ('capitalsolutionsgroup'),
    ('captiv8'),
    ('captivateiq'),
    ('carbon'),
    ('carbon6technologies'),
    ('carboncapture'),
    ('carbondirect'),
    ('carbonhealth'),
    ('cardanofoundation'),
    ('cardata'),
    ('cardiologs'),
    ('cardiosense'),
    ('cardless'),
    ('careaccessresearch'),
    ('careernowbrands'),
    ('careerstaffing'),
    ('caremessage'),
    ('careology'),
    ('carestino'),
    ('carewest'),
    ('cargo-2'),
    ('cargo-partner'),
    ('carismatherapeutics'),
    ('carma'),
    ('carolinacenterforaba'),
    ('cart'),
    ('carta'),
    ('cartodb'),
    ('cartography'),
    ('cartrawler'),
    ('carusoaffiliated'),
    ('casa'),
    ('cascade-climate'),
    ('casdincapital'),
    ('casetext'),
    ('caseware'),
    ('cashrewards'),
    ('caslservice'),
    ('catalist'),
    ('catalyze'),
    ('catbird'),
    ('catenamedia'),
    ('catlabs'),
    ('causal-app'),
    ('cbtnuggets'),
    ('ccmr3'),
    ('cdcfoundation'),
    ('cef'),
    ('celaralabs'),
    ('celerion'),
    ('celestia'),
    ('cellares'),
    ('celo'),
    ('centerfield'),
    ('centml'),
    ('centrifuge'),
    ('centro'),
    ('cents'),
    ('centuria'),
    ('cere-network'),
    ('cerevel-2'),
    ('certifid'),
    ('certifyos'),
    ('certik'),
    ('cesiumastro'),
    ('cfgi'),
    ('cfsenergy'),
    ('cgsfederal'),
    ('chain'),
    ('chainlink'),
    ('chamberofprogress'),
    ('change'),
    ('channelape'),
    ('chargelab-inc'),
    ('chargezoom'),
    ('chariot'),
    ('charitywater'),
    ('charmindustrial'),
    ('charter-impact'),
    ('chartio'),
    ('chatmeter'),
    ('checkmate'),
    ('checkout'),
    ('cheerz'),
    ('chefrobotics'),
    ('cherre'),
    ('cherrytechnologies'),
    ('chestnutcarbon'),
    ('chicago2024'),
    ('chironhealth'),
    ('chooose'),
    ('chooosejobs'),
    ('choose'),
    ('chownow'),
    ('chronicled-2'),
    ('churchatthepark'),
    ('chyronhego'),
    ('ciandt'),
    ('cic'),
    ('cimgroup'),
    ('cin7'),
    ('cinchy'),
    ('ciphertrace'),
    ('circadiahealth'),
    ('circleco'),
    ('circlemedical'),
    ('circlepharma'),
    ('circonus'),
    ('circulo'),
    ('cirquedusoleil'),
    ('cision'),
    ('citadelsecurityusa'),
    ('citcon'),
    ('citrineinformatics'),
    ('civitech'),
    ('civitta'),
    ('ciwebgroup'),
    ('clabs'),
    ('clari'),
    ('clarifyhealth'),
    ('clasp'),
    ('class'),
    ('class101'),
    ('cleanspark'),
    ('clearbit'),
    ('clearcapital'),
    ('clearedge'),
    ('clearer'),
    ('clearpoint'),
    ('clerky'),
    ('clevertap'),
    ('clicktime'),
    ('climate-collaborative'),
    ('climateai'),
    ('climatepower'),
    ('climbcredit'),
    ('clinicalhealthnetworkfortransformation'),
    ('clipbook'),
    ('cloaked-app'),
    ('cloud-academy'),
    ('cloudinary'),
    ('cloudnc'),
    ('cloudwalk'),
    ('clovirtualfashion'),
    ('clozd'),
    ('clubassist'),
    ('clubhouse'),
    ('cmgx'),
    ('cnmllp'),
    ('coalfire'),
    ('coatesgroup'),
    ('cobaltrobotics'),
    ('cocodelivery'),
    ('cocollective'),
    ('coda'),
    ('codapayments'),
    ('code2040'),
    ('codecombat'),
    ('codecov'),
    ('codelitt'),
    ('coderpad'),
    ('codesee'),
    ('codeway'),
    ('codewaystudios'),
    ('cofactr'),
    ('coffeemeetsbagel'),
    ('coforma'),
    ('cogentanalytics'),
    ('cogentbiosciences'),
    ('cogility'),
    ('cogitocorp'),
    ('cognite'),
    ('cognitiv'),
    ('cohere'),
    ('coil'),
    ('coiled'),
    ('coinflip'),
    ('coingecko'),
    ('coinmarketcap'),
    ('coins'),
    ('coinshift'),
    ('coinsquare'),
    ('cointracker'),
    ('colibrigroup'),
    ('collabora'),
    ('collate'),
    ('collectivei'),
    ('collectlyinc'),
    ('collegevine'),
    ('color'),
    ('coloradocoalition'),
    ('colorofchange'),
    ('combo'),
    ('commencis'),
    ('commentsold'),
    ('common'),
    ('common-networks'),
    ('commoncause'),
    ('commoncode'),
    ('commonfuture'),
    ('commonlit'),
    ('communityactionhouse'),
    ('communityboost'),
    ('companial'),
    ('companyon'),
    ('compasslexecon'),
    ('compassx'),
    ('complex'),
    ('comply'),
    ('componentrepairtechnologies'),
    ('composer'),
    ('compound-2'),
    ('compstak'),
    ('computercare'),
    ('comtravo'),
    ('concordnow'),
    ('concurrency'),
    ('conduit'),
    ('conduktor'),
    ('confluent'),
    ('confluera'),
    ('conjecture'),
    ('connectassistance'),
    ('connectly'),
    ('connectrn'),
    ('connexiatelerad'),
    ('connext-network'),
    ('connorgp'),
    ('constellation'),
    ('contabilizei'),
    ('contactout'),
    ('container-solutions'),
    ('contentsquare'),
    ('continuumglobal'),
    ('continuumindustries'),
    ('contra'),
    ('contrastsecurity'),
    ('controlup'),
    ('convai-technologies-inc'),
    ('convelio'),
    ('convergentresearch'),
    ('conversantgroup'),
    ('conversenow'),
    ('conversica'),
    ('copia'),
    ('copiapower'),
    ('copilotmoney'),
    ('copper'),
    ('copyai'),
    ('corbalt'),
    ('cordtechnologies'),
    ('corebts'),
    ('coredigitalmedia'),
    ('coreresponse'),
    ('cority'),
    ('cornershopapp'),
    ('cos'),
    ('cottage'),
    ('couchbase'),
    ('coupa'),
    ('coursera'),
    ('court-avenue'),
    ('covariant'),
    ('cover-2'),
    ('covergenius'),
    ('cprime'),
    ('craft'),
    ('craftwater'),
    ('craftwaterinc'),
    ('cratejoy'),
    ('creadits'),
    ('creativemarket'),
    ('creatordeck'),
    ('creatoriq'),
    ('cred'),
    ('credavenue'),
    ('creditkarma'),
    ('credo'),
    ('credplatform'),
    ('cresta'),
    ('crestoperations'),
    ('crewapp'),
    ('crewbloom'),
    ('crgo'),
    ('crickethealth'),
    ('criticaltilt'),
    ('crosscountry-consulting'),
    ('crossfit'),
    ('crossing-minds'),
    ('crosslaketech'),
    ('crossvale'),
    ('crowdriff'),
    ('cruise'),
    ('crumbl'),
    ('crunchbase'),
    ('crusoeenergy'),
    ('cruxinformatics'),
    ('cruxocm'),
    ('crypto'),
    ('crypto-innovation'),
    ('crytek'),
    ('csaassociates'),
    ('cscgeneration-2'),
    ('csit'),
    ('cti-md'),
    ('cubegroup'),
    ('culdesac'),
    ('cullinanoncology'),
    ('culturekings'),
    ('culturetrip'),
    ('cuningham'),
    ('curai'),
    ('curbio'),
    ('curefit'),
    ('curieco'),
    ('curimeta'),
    ('curiositymedia'),
    ('curology'),
    ('currencycloud'),
    ('curri'),
    ('custodiabank'),
    ('cx2'),
    ('cxnpl'),
    ('cxtsoftware'),
    ('cyara'),
    ('cybcube'),
    ('cybercare'),
    ('cyberfame'),
    ('cyberonesecurity'),
    ('cybourn'),
    ('cyderes'),
    ('cye'),
    ('cygn'),
    ('cyngn'),
    ('cyvl-ai'),
    ('d-edge'),
    ('d-fendsolutions'),
    ('dabsinc'),
    ('dadavidson'),
    ('daedalean'),
    ('dailywire'),
    ('daniels-sharpsmart'),
    ('daos-hub'),
    ('darkslope'),
    ('darwinhomes'),
    ('dasservicesllc'),
    ('databricks'),
    ('datahawk'),
    ('dataiku'),
    ('datalabusa'),
    ('dataplor'),
    ('dave'),
    ('dawnlight'),
    ('dayblinkconsulting'),
    ('dayonebio'),
    ('dazn'),
    ('dbtlabs'),
    ('dds'),
    ('dealerware'),
    ('dealmaker'),
    ('dealpath'),
    ('debtsy'),
    ('decathlon'),
    ('decentraland'),
    ('decilegroup'),
    ('deep6'),
    ('deepcell'),
    ('deepgenomics'),
    ('deepgram'),
    ('deepjudge'),
    ('deepsky'),
    ('defialliance'),
    ('deft'),
    ('degreed'),
    ('deleteme'),
    ('delfidiagnostics'),
    ('deliverect'),
    ('deliveryrelay'),
    ('deloitte'),
    ('delphix'),
    ('deltadental'),
    ('deltasands'),
    ('delv'),
    ('demiurgestudios'),
    ('dempseyuniform-2'),
    ('density'),
    ('deputy'),
    ('desafiolatam'),
    ('desbytech'),
    ('descript-2'),
    ('designpickle'),
    ('desmos'),
    ('despegar'),
    ('destinationknot'),
    ('deuna'),
    ('devcycle'),
    ('developintelligence'),
    ('device42'),
    ('devoted'),
    ('devsoftam'),
    ('dexcarehealth'),
    ('dexterity'),
    ('dga'),
    ('dgraph'),
    ('dialogdesign'),
    ('diamondage3d'),
    ('diamondfoundry'),
    ('didi-global'),
    ('didomi'),
    ('digimarc'),
    ('digital-matter'),
    ('digitalmass'),
    ('digitalmediamanagement'),
    ('digitalremedy'),
    ('digitalturbine'),
    ('dijital-team-pty-ltd'),
    ('dimo'),
    ('diool'),
    ('dioxycle'),
    ('directi'),
    ('disher'),
    ('dispo'),
    ('disqo'),
    ('disruptivegames'),
    ('distru'),
    ('divergent3d'),
    ('diversified-automation'),
    ('diversifiedradiology'),
    ('diversio'),
    ('dixa'),
    ('djeholdings'),
    ('dlocal'),
    ('dnagenotek'),
    ('dnb'),
    ('dnc'),
    ('dnllc'),
    ('dntanks'),
    ('dnwest'),
    ('docebo'),
    ('dockwa'),
    ('docsumo'),
    ('dodmg'),
    ('dogdrop'),
    ('dollskill'),
    ('domio'),
    ('done'),
    ('doola'),
    ('doppler'),
    ('dorahacks'),
    ('doranjones'),
    ('dott'),
    ('doubledown'),
    ('doxel'),
    ('dozee'),
    ('dragonarmy'),
    ('drchrono'),
    ('dreamgames'),
    ('dreamgolf'),
    ('dreamsports'),
    ('drip'),
    ('drivekyte'),
    ('drivemode'),
    ('drivetrain'),
    ('drivy'),
    ('drizly'),
    ('dronedeploy'),
    ('dronesense'),
    ('dru-app'),
    ('dscout'),
    ('dsi'),
    ('dten'),
    ('duetti'),
    ('duffel'),
    ('dulcedo'),
    ('duolingo'),
    ('dutch'),
    ('dwavesys'),
    ('dwolla'),
    ('dydx-ops'),
    ('dynamiccatholic'),
    ('dynamoai'),
    ('dynastystudios'),
    ('dynatrace'),
    ('dynavax'),
    ('dyte-io'),
    ('dzen'),
    ('e-core'),
    ('e2optics'),
    ('eargo'),
    ('earlybird'),
    ('earthspecies'),
    ('eastwall'),
    ('easyagile'),
    ('easybrain'),
    ('easypost-2'),
    ('easystreetcap'),
    ('eaze'),
    ('ebury'),
    ('ecocart'),
    ('economicmodeling'),
    ('econstruct'),
    ('ecosia'),
    ('edacious'),
    ('edenhealth'),
    ('edera'),
    ('edge-impulse'),
    ('edgeofs'),
    ('edgrp'),
    ('edisonenergy'),
    ('editorialist'),
    ('edpuzzle'),
    ('educationatwork'),
    ('educative'),
    ('efront'),
    ('egen'),
    ('egenesisbio'),
    ('egensolutions'),
    ('eh2'),
    ('einride'),
    ('ekimetrics'),
    ('ekohealth'),
    ('elearningmind'),
    ('electricmind'),
    ('electrifiedthermalsolutions'),
    ('electrum'),
    ('eleks'),
    ('elementsolutions'),
    ('elementus'),
    ('elevatelabs'),
    ('elfbeauty'),
    ('eliyan'),
    ('ellevationeducation'),
    ('elliemae'),
    ('elliveproductions'),
    ('elodiegames'),
    ('elroyair'),
    ('elty'),
    ('elvah'),
    ('emburse'),
    ('emeraldtherapeutics'),
    ('emersoncollective'),
    ('emersonhc'),
    ('emesent'),
    ('emilabs'),
    ('emilyslist'),
    ('emitwise'),
    ('emma-sleep'),
    ('empiricotx'),
    ('empowerly'),
    ('empowerpartnerships'),
    ('enable'),
    ('enablecomp'),
    ('enarahealth'),
    ('encore-renewable-energy'),
    ('encorevet'),
    ('end'),
    ('endless'),
    ('endlesswest'),
    ('endpointclinical'),
    ('endpointclosing'),
    ('endycanada'),
    ('energeticcapital'),
    ('energy-by-5'),
    ('energy-impact'),
    ('energyrecovery'),
    ('energyvault'),
    ('enertechsearch'),
    ('engine'),
    ('engine-ventures'),
    ('enjoybloom'),
    ('enlitic'),
    ('enso'),
    ('entangledtalent'),
    ('entanglelabs'),
    ('entefy'),
    ('enter-rcm-llc'),
    ('entrata'),
    ('envato-2'),
    ('envedabio'),
    ('envisionemploymentsolutions'),
    ('enya'),
    ('epicio'),
    ('epifi'),
    ('episci'),
    ('epsilon3'),
    ('eqbank'),
    ('equativ'),
    ('equiphealth'),
    ('er-meds'),
    ('erg'),
    ('erpsuccesspartners'),
    ('esalen'),
    ('espace'),
    ('esper'),
    ('esper-3'),
    ('espresso'),
    ('esrtreit'),
    ('estenda'),
    ('eternal'),
    ('ethena'),
    ('ethenalabs'),
    ('ether-fi'),
    ('ethereumfoundation'),
    ('ethos'),
    ('ethree'),
    ('etsy'),
    ('eudia'),
    ('eve'),
    ('evenfinancial'),
    ('eventbrite'),
    ('everbridge'),
    ('everc'),
    ('evergreenstrategygroup'),
    ('evergrow'),
    ('everlane'),
    ('everlaw'),
    ('everli'),
    ('everlywell'),
    ('evidentid'),
    ('evisort-2'),
    ('evocalize'),
    ('evr'),
    ('evrealty-us'),
    ('exa-labs'),
    ('exabeam'),
    ('exowatt'),
    ('exp'),
    ('explodingkittens'),
    ('explorejobssearch'),
    ('expopulus'),
    ('extremenetworks'),
    ('extropic'),
    ('exygy'),
    ('ey-ps'),
    ('ezcater'),
    ('f16y'),
    ('f8-federal'),
    ('fabertechnologies'),
    ('fabric'),
    ('fabrica'),
    ('factor'),
    ('factored'),
    ('fairmat'),
    ('fairmatic'),
    ('fama'),
    ('fampay'),
    ('fanatee'),
    ('fanatics'),
    ('fantasy'),
    ('farcast'),
    ('farfetch'),
    ('farmgirlflowers'),
    ('farmwise'),
    ('fast'),
    ('fatetherapeutics'),
    ('fathom5'),
    ('fathomhealth'),
    ('fathomradiant'),
    ('fazzfinancial'),
    ('federato'),
    ('feedmedia'),
    ('fehrandpeers'),
    ('feldinc'),
    ('fellowshiplifeinc'),
    ('femalefounders-fund'),
    ('fevo'),
    ('ffws'),
    ('fi'),
    ('fiddlerlabs'),
    ('field-ai'),
    ('fieldnation'),
    ('figfinancial'),
    ('fightcamp'),
    ('figma'),
    ('filevine'),
    ('financeinabox'),
    ('finch'),
    ('finchloom'),
    ('find'),
    ('findem'),
    ('findhelp'),
    ('findigs'),
    ('findmine'),
    ('findora'),
    ('finfit'),
    ('finix'),
    ('finleytechnologies'),
    ('finn'),
    ('finquery'),
    ('finstek'),
    ('fintual'),
    ('finwavesemiconductor'),
    ('fireflyon'),
    ('firemon'),
    ('firewoodmarketing'),
    ('fireworkhq'),
    ('firmex'),
    ('firsthand'),
    ('firstround'),
    ('firstup'),
    ('fiscalnote'),
    ('fishtownanalytics'),
    ('fitbod'),
    ('fitifyworkouts'),
    ('fitzmark'),
    ('fixinssoulkitchen'),
    ('fizz'),
    ('flaretherapeutics'),
    ('flash'),
    ('flashapp'),
    ('flatfile'),
    ('flatironsdevelopment'),
    ('flawlessai'),
    ('fleetsmith'),
    ('flex'),
    ('flexcollegeprep'),
    ('flexiti'),
    ('fliff'),
    ('flightcx'),
    ('flipfit'),
    ('flipster'),
    ('flockjay'),
    ('floqast'),
    ('florence'),
    ('floridadems'),
    ('flowerave'),
    ('flowfoundation'),
    ('flowlife'),
    ('flowspace'),
    ('flpstudio'),
    ('fluence'),
    ('flux-mobility'),
    ('fluxergy-2'),
    ('flynncompanies'),
    ('flyrhospitality'),
    ('flyrlabs'),
    ('flytographer'),
    ('flywheel-2'),
    ('flywheeldigital'),
    ('flyzipline'),
    ('fnatic'),
    ('focuspartners'),
    ('fond'),
    ('food52'),
    ('foodsmart'),
    ('foodstuffs'),
    ('forbrightbank'),
    ('forestcarbonworks'),
    ('form'),
    ('formant'),
    ('format'),
    ('formelife'),
    ('formenergy'),
    ('formidable'),
    ('formlogic'),
    ('formstack'),
    ('formulamonks'),
    ('forourfuturefund'),
    ('fortahealth'),
    ('forterra'),
    ('forto'),
    ('fortreum'),
    ('fortuneschool'),
    ('forusall'),
    ('foth'),
    ('founderscard'),
    ('foundersfactory'),
    ('foundersfactoryafrica'),
    ('foundry-ai'),
    ('fourth'),
    ('foxen'),
    ('foxitsoftware'),
    ('foxtrotco'),
    ('fp'),
    ('fractional'),
    ('framework'),
    ('franklinai'),
    ('freedompay'),
    ('freedomprep'),
    ('freedomsolarpower'),
    ('freeletics'),
    ('freiheit-2'),
    ('fresha'),
    ('freshhealth'),
    ('freshly'),
    ('frontapp'),
    ('frontify'),
    ('frontsteps'),
    ('frubana'),
    ('ftc'),
    ('ftxus'),
    ('fuellabs'),
    ('fulcrumpro'),
    ('fulcrumservices'),
    ('fullscreen'),
    ('fullscript'),
    ('fullspectrumsoftware'),
    ('fullstacklabs'),
    ('fullyvested'),
    ('fundapps'),
    ('fundera'),
    ('fundrise'),
    ('funxyz'),
    ('fuse'),
    ('futurelaboratori'),
    ('futureof-life'),
    ('fwgsolutions'),
    ('fynd'),
    ('fyusion'),
    ('g2i'),
    ('gagezero'),
    ('galatea-associates'),
    ('galepartners'),
    ('galvanick'),
    ('gametime'),
    ('gamma'),
    ('gantri'),
    ('gantry'),
    ('gate'),
    ('gather-health'),
    ('gatherai'),
    ('gatheround'),
    ('gatorbio'),
    ('gauntlet'),
    ('gausslabs'),
    ('gbatteries'),
    ('gdi'),
    ('gearset'),
    ('geckoboard'),
    ('genbio'),
    ('genedit'),
    ('generallyintelligent'),
    ('generation'),
    ('genesis'),
    ('genesistherapeutics'),
    ('genome-medical'),
    ('genomicsplc'),
    ('gentibio'),
    ('geocomply-2'),
    ('geophy'),
    ('geotab'),
    ('gess'),
    ('getansa'),
    ('getaround'),
    ('getbuilt'),
    ('getcenter'),
    ('getcircuit'),
    ('getcollaborative'),
    ('getdeardoc'),
    ('getdimension'),
    ('getduranta'),
    ('getfable'),
    ('getjerry'),
    ('getjones'),
    ('getlabs'),
    ('getlago'),
    ('getmaple'),
    ('getmetronome'),
    ('getmidas'),
    ('getmulberry'),
    ('getmysa'),
    ('getpatch'),
    ('getsquire'),
    ('gettyimages'),
    ('getwingapp'),
    ('getzuma'),
    ('ghcorps'),
    ('ghj'),
    ('ghjadvisors'),
    ('ghsmartjobs'),
    ('gigster'),
    ('ginkgobioworks'),
    ('girlswhocode'),
    ('gitbook'),
    ('gitlab'),
    ('givax'),
    ('givebutter'),
    ('givingli'),
    ('glass-health-inc'),
    ('glassnode'),
    ('glide'),
    ('glidian'),
    ('glints'),
    ('glitch'),
    ('global-elite'),
    ('globalelitecareers'),
    ('globalstrategygroup'),
    ('gloo'),
    ('glowforge'),
    ('glsllc'),
    ('glu'),
    ('gluegroups'),
    ('gmo'),
    ('go-cloudforce'),
    ('go2-markets'),
    ('goabstract'),
    ('goalbookapp-2'),
    ('goat'),
    ('gobrightside'),
    ('gocatalant'),
    ('goconsensus'),
    ('goeverbright'),
    ('goforward'),
    ('gogolook'),
    ('gohighlevel'),
    ('gojek'),
    ('gojob'),
    ('gokenamerica1'),
    ('goldcast'),
    ('goldfinch'),
    ('golfscopeinc'),
    ('golucid'),
    ('gomaterials'),
    ('gonetspeed'),
    ('gong'),
    ('good-party'),
    ('goodamerican'),
    ('goodeggs'),
    ('goodgamestudios'),
    ('goodleap'),
    ('goodrx'),
    ('gopuff'),
    ('gordian-bio'),
    ('gorgias'),
    ('gorh'),
    ('gorillalogic'),
    ('goshippo'),
    ('gostudion'),
    ('goswift'),
    ('gotit'),
    ('gotogroup'),
    ('gouconnect'),
    ('gouspack'),
    ('govini'),
    ('gowanco'),
    ('gpsfx'),
    ('gr0'),
    ('grab'),
    ('grafana'),
    ('grailbio'),
    ('grand'),
    ('grandrounds'),
    ('granicus'),
    ('granular'),
    ('grapesworldwide'),
    ('grav'),
    ('gravie'),
    ('gravisrobotics'),
    ('graviticsspace'),
    ('graylog'),
    ('grayscale'),
    ('great-gray-group'),
    ('greenlight'),
    ('greenroompr'),
    ('greenshieldholdings'),
    ('greybrook'),
    ('grid'),
    ('gridmatic'),
    ('gridware'),
    ('grimm-co'),
    ('groupbyinc'),
    ('groupthink'),
    ('grove'),
    ('grow'),
    ('growthlending'),
    ('growthtribe'),
    ('gscloudsolutions'),
    ('gsr'),
    ('gts'),
    ('guidestareldercare'),
    ('guidewire'),
    ('guild'),
    ('gurobioptimization'),
    ('gursey'),
    ('gushwork'),
    ('guusto'),
    ('gynger'),
    ('h1'),
    ('habito'),
    ('hackerone'),
    ('hadrian'),
    ('hakkoda'),
    ('hallow'),
    ('haloeurope'),
    ('halosbodycams'),
    ('halter'),
    ('hamiltonps'),
    ('handoff'),
    ('hannaandersson'),
    ('hap-capital'),
    ('happn'),
    ('happyco'),
    ('happyfresh'),
    ('happyhiller'),
    ('happymoney'),
    ('harmonicdesign'),
    ('harmony'),
    ('harness'),
    ('harrisinsights'),
    ('harrison'),
    ('harrisonst'),
    ('harvest-2'),
    ('hash'),
    ('hashtagpaid'),
    ('hasi'),
    ('hatch'),
    ('hatchcard'),
    ('hatchit'),
    ('haus'),
    ('havenparkcommunities'),
    ('hawaiianhost'),
    ('hawkemedia'),
    ('hazelanalytics'),
    ('hcvt'),
    ('hdvi'),
    ('hdworks'),
    ('headhunters'),
    ('headout'),
    ('headroyce'),
    ('headspace'),
    ('health-atscale'),
    ('health-match'),
    ('healthatom'),
    ('healthcare'),
    ('healthebay'),
    ('healthline'),
    ('healthtensor'),
    ('healthverity'),
    ('healx'),
    ('heard'),
    ('hearsaysystems'),
    ('heartbeathealth'),
    ('heartflow'),
    ('heaten-germany-gmbh'),
    ('heetch'),
    ('heirloomtechnologies'),
    ('helbiz'),
    ('hellobestow'),
    ('hellojagger'),
    ('hellonanny'),
    ('hellopicnic'),
    ('hellosign'),
    ('helm'),
    ('helpscout'),
    ('helpshift'),
    ('helsing'),
    ('herbl'),
    ('heretic-fund'),
    ('hermeus'),
    ('hevodata'),
    ('hexa'),
    ('hexagonusfederal'),
    ('heyjane'),
    ('heyrowan'),
    ('hgen'),
    ('hhaexchange'),
    ('hicleo'),
    ('highbarhealth'),
    ('highfidelity'),
    ('highlandfleets-2'),
    ('highlight'),
    ('highspot'),
    ('hightechhigh'),
    ('highwirepr'),
    ('hihello'),
    ('hike'),
    ('himama'),
    ('hingehealth'),
    ('hint'),
    ('hipcamp'),
    ('hireincloud'),
    ('hirezstudios'),
    ('hiring'),
    ('hiro'),
    ('hive'),
    ('hivebrite'),
    ('hively'),
    ('hivemapper'),
    ('hivemind-capital'),
    ('hiver'),
    ('hlkagency'),
    ('hmbl-tech'),
    ('holmusk'),
    ('hologram'),
    ('holygrail'),
    ('homechef'),
    ('homelister'),
    ('homesnap'),
    ('homestars'),
    ('homex'),
    ('hone'),
    ('honeybeerobotics'),
    ('honeycomb'),
    ('honeypot'),
    ('honkforhelp'),
    ('hopelab'),
    ('hopeservices'),
    ('hophr'),
    ('hopper'),
    ('hopthru'),
    ('horizon'),
    ('hostinger'),
    ('hoteltonight'),
    ('hotstar'),
    ('hottopic'),
    ('houzz'),
    ('howgood'),
    ('hqxyz'),
    ('hr-hints'),
    ('hubla'),
    ('huckleberrylabs'),
    ('huma'),
    ('human-protocol-foundation'),
    ('humanapi'),
    ('humanetech'),
    ('humanfirst'),
    ('humanityprotocol'),
    ('humansecurity'),
    ('humata'),
    ('humi-2'),
    ('hungerfreeamerica'),
    ('hungryharvest'),
    ('hvmn'),
    ('hydrow'),
    ('hypebeast'),
    ('hypercomply'),
    ('hyperlane'),
    ('hyperscience'),
    ('hyperspectral'),
    ('hyundai-aptivad'),
    ('ianulytalentaccelerators'),
    ('icemortgagetechnology'),
    ('icertis'),
    ('icodde'),
    ('iconicair'),
    ('icosavax'),
    ('ideagen'),
    ('idealistconsulting'),
    ('idealscorp'),
    ('ideasunited'),
    ('ideayabio'),
    ('idmworks'),
    ('idt'),
    ('if7'),
    ('ifm-us'),
    ('igmbio'),
    ('ignite'),
    ('illumination'),
    ('illumix'),
    ('imagexmedia'),
    ('imbue'),
    ('imentor'),
    ('immiland-canada-inc'),
    ('immuta'),
    ('immutable'),
    ('imo-online'),
    ('imperfectfoods'),
    ('imperfectproduce'),
    ('impossiblecloud'),
    ('impossiblefoods'),
    ('improbable'),
    ('improvadoio'),
    ('inato'),
    ('inboxlab'),
    ('incandescent'),
    ('includedhealth'),
    ('incompasshealth'),
    ('incorta'),
    ('indebted'),
    ('indico'),
    ('indigoag'),
    ('indivisible'),
    ('inductiveautomation'),
    ('inductivehealth'),
    ('industrydive'),
    ('infieldstrategies'),
    ('infinigods'),
    ('infinit'),
    ('infinitus'),
    ('inflection'),
    ('inflexion'),
    ('inflow'),
    ('inflowfed'),
    ('influ2'),
    ('influur'),
    ('infogrid'),
    ('infogridlinks'),
    ('infraservices'),
    ('infrastructureandcapitalprojects'),
    ('infrastructureontario'),
    ('infstones'),
    ('initialized'),
    ('injectivelabs'),
    ('inkitt'),
    ('inlight'),
    ('innablr'),
    ('innocraft'),
    ('innovafeed'),
    ('innovationdept'),
    ('innovativesol-2'),
    ('inpay-2'),
    ('insify'),
    ('insightm'),
    ('insomniacdesign'),
    ('insomniacookies'),
    ('inspiration-mobility'),
    ('instawork'),
    ('instituteforproteininnovation'),
    ('instructure'),
    ('instrument'),
    ('instrumental'),
    ('instrumentl'),
    ('integrabeauty'),
    ('integrate'),
    ('intelligent-city'),
    ('intelliware'),
    ('intelycare'),
    ('intenseye'),
    ('interaxon'),
    ('interfolio'),
    ('interiormarketinggroup'),
    ('interlude'),
    ('internationalbudget'),
    ('intersect'),
    ('intersectpower'),
    ('intive'),
    ('intouchgroup'),
    ('intrafi'),
    ('intrepid'),
    ('intrepidcollegeprep'),
    ('intricately'),
    ('intro'),
    ('intropic'),
    ('inventa'),
    ('investorflow'),
    ('invinity'),
    ('invisible'),
    ('invisible-ai'),
    ('invokestaffing'),
    ('inworld'),
    ('iobeya'),
    ('ion'),
    ('ionic'),
    ('ionicpartners'),
    ('iovlabs'),
    ('iphi'),
    ('ippon'),
    ('ircagroup'),
    ('iron_hack'),
    ('ironcladapp'),
    ('ironnetcybersecurity'),
    ('ironox'),
    ('isee'),
    ('isometrictechnologies'),
    ('ispace-inc'),
    ('issuu'),
    ('italic'),
    ('iterative'),
    ('itutor'),
    ('ivo'),
    ('iyzico'),
    ('izotope'),
    ('jacquard'),
    ('jagex'),
    ('jamcity'),
    ('jane'),
    ('janeapp'),
    ('januaryinc'),
    ('januxrx'),
    ('jar-app'),
    ('jarofleets'),
    ('jarvis'),
    ('jaspertx'),
    ('jeff-app'),
    ('jellysmack'),
    ('jetsupport'),
    ('jina-ai'),
    ('jiostar'),
    ('jito'),
    ('jitxinc'),
    ('jmawireless'),
    ('jmi'),
    ('jobandtalent'),
    ('jobcase'),
    ('jobgether'),
    ('jobscan-2'),
    ('jockmkt'),
    ('joebiden'),
    ('joelefrank'),
    ('joinclubhouse'),
    ('joindrop'),
    ('joinefnow'),
    ('joingraphite'),
    ('joinlincoln'),
    ('joinmonument-2'),
    ('joinpapa'),
    ('joinroot'),
    ('joinswoop'),
    ('jointopo'),
    ('joinzoe'),
    ('joltcharge'),
    ('joshstoysandgames'),
    ('journeyclinical'),
    ('joyrideautos'),
    ('jumpcloud'),
    ('juneshine'),
    ('juniperplatform'),
    ('juno'),
    ('junoniapartners'),
    ('jupe'),
    ('jupiter'),
    ('jupiterintel'),
    ('jupiterone'),
    ('juro'),
    ('jushico'),
    ('justwatch'),
    ('justworkslabs'),
    ('kabam'),
    ('kafene'),
    ('kaiahealth'),
    ('kaiber'),
    ('kaironlabs'),
    ('kairosaerospace'),
    ('kaizenplatform'),
    ('kamcord'),
    ('kandji'),
    ('kaporcapital'),
    ('kapta-space'),
    ('kapwing'),
    ('karat'),
    ('kariusdx'),
    ('karmacheck'),
    ('kasada'),
    ('kasheesh'),
    ('kava'),
    ('kavalabs'),
    ('kcasbio'),
    ('kddia'),
    ('kdinfotech'),
    ('keboola'),
    ('keepit'),
    ('keeptruckin'),
    ('kellytelecom'),
    ('keloniatx'),
    ('kenko'),
    ('kensho'),
    ('kepler'),
    ('kerishullteam'),
    ('ketch'),
    ('kevala'),
    ('keyloop'),
    ('kg'),
    ('kgroupcompanies'),
    ('kibeam'),
    ('kickback'),
    ('kickmaker'),
    ('kickoff'),
    ('kickstart-ent'),
    ('kickstarter'),
    ('kiddom'),
    ('kimiatherapeutics'),
    ('kingfish-company'),
    ('kingfishgroup'),
    ('kinnek'),
    ('kinsa'),
    ('kinsta'),
    ('kippsocal'),
    ('kitcheck'),
    ('kitmanlabs'),
    ('kitopi'),
    ('kitware'),
    ('kivaai'),
    ('kivaconfections'),
    ('kiwico'),
    ('kkbox'),
    ('klarna'),
    ('klaviyo'),
    ('klearnow'),
    ('klick'),
    ('klubworks'),
    ('klue'),
    ('kmaconsultingllc'),
    ('knix'),
    ('knock'),
    ('knockcrm'),
    ('knotch'),
    ('knownwell'),
    ('knrtherapy'),
    ('koalahealth'),
    ('kobie'),
    ('kochava'),
    ('koddi'),
    ('kodex'),
    ('kodiak'),
    ('kogan'),
    ('kogniz'),
    ('koho'),
    ('kokonetworks'),
    ('koldpartners'),
    ('kolibrigames'),
    ('kolsquare'),
    ('kong'),
    ('kontakt'),
    ('konviere'),
    ('koombea'),
    ('kooptechnologies'),
    ('koreaboo'),
    ('korrobio'),
    ('kotm'),
    ('kpaonline'),
    ('kpler'),
    ('kpmg-2'),
    ('kpmgnz'),
    ('kraken'),
    ('kraken123'),
    ('kruzeconsulting'),
    ('kubecost'),
    ('kubra'),
    ('kueski'),
    ('kumo'),
    ('kumospace'),
    ('kungfu'),
    ('kwanso'),
    ('kwh-analytics'),
    ('kwx'),
    ('kyivstar'),
    ('kyruus'),
    ('kyruushealth'),
    ('kyte'),
    ('kyverna'),
    ('laagencia'),
    ('labelbox'),
    ('lacunatechnologies'),
    ('ladders'),
    ('lahaus'),
    ('lalamove'),
    ('lambdal'),
    ('lambdaschool'),
    ('laminarprojects'),
    ('laminiai'),
    ('lamudi'),
    ('landing'),
    ('landmarkbio'),
    ('lansingschools'),
    ('larian'),
    ('laserhub-2'),
    ('last-energy'),
    ('last9'),
    ('latch'),
    ('latitudeinc'),
    ('latticehq'),
    ('launchdarkly'),
    ('launcherspace'),
    ('launchhouse'),
    ('launchhousing'),
    ('launchsquad'),
    ('lavictoria'),
    ('layup'),
    ('lcbcchurch'),
    ('leadbank'),
    ('leadgenius'),
    ('leadr'),
    ('leadsonline'),
    ('leaf-trade'),
    ('leaflogistics'),
    ('leantaas'),
    ('leap-2'),
    ('learnlux'),
    ('learnwithjuni'),
    ('leasequery'),
    ('ledger'),
    ('ledn'),
    ('lefonds-du-bien-commun'),
    ('legacy'),
    ('legacyrestorationllc'),
    ('legacyservicepartners'),
    ('legalfundingcentral'),
    ('legend'),
    ('legitimatetech'),
    ('lempire'),
    ('lemurian-labs'),
    ('lendbuzz'),
    ('lendinghome'),
    ('lendio'),
    ('leolabs-2'),
    ('leotechnologies'),
    ('lessen'),
    ('letote'),
    ('leucine'),
    ('levelai'),
    ('levelup'),
    ('lever'),
    ('levvel'),
    ('lexeotx'),
    ('lexingtonmedical'),
    ('liatrio'),
    ('liberatii'),
    ('life'),
    ('lifeforce'),
    ('lifen'),
    ('lifestance'),
    ('lightci'),
    ('lightedge'),
    ('lightship'),
    ('lillio'),
    ('limberhealth'),
    ('lime'),
    ('limebike'),
    ('liminal'),
    ('limitbreak'),
    ('limitless'),
    ('lindblad-expeditions'),
    ('linea'),
    ('lingoda'),
    ('linksys'),
    ('linktree'),
    ('linq'),
    ('linuxacademy'),
    ('lionheartkid'),
    ('lirvanalabs'),
    ('littleotter'),
    ('littlesprouts'),
    ('livechatinc'),
    ('livefront'),
    ('livekindred'),
    ('livelike'),
    ('liveoakfiber'),
    ('livingcarbon'),
    ('livinghr'),
    ('loadsmart'),
    ('loancrate'),
    ('loblawagency'),
    ('loblawdigital'),
    ('localfoodgroup'),
    ('lochgroup'),
    ('lodgify'),
    ('loft'),
    ('loftorbital'),
    ('logically'),
    ('logicmanager'),
    ('logikcull'),
    ('login'),
    ('logistimo'),
    ('logixboard'),
    ('logrocket'),
    ('logz'),
    ('longtail-ai'),
    ('longview'),
    ('longwall'),
    ('looker'),
    ('loop-health'),
    ('loopinternet'),
    ('loopio'),
    ('loopreturns'),
    ('loupetheapp'),
    ('lovepop'),
    ('lovepopcards'),
    ('loyalfordogs'),
    ('loyaltylion'),
    ('lpainc'),
    ('lqdigital'),
    ('lrymediagroup'),
    ('lsa-hr'),
    ('lsaweb'),
    ('ltaresearch'),
    ('ltse'),
    ('lucasmuseum'),
    ('lucca'),
    ('lucid'),
    ('lucidmotors'),
    ('lucidworks'),
    ('lulafit'),
    ('lumaai'),
    ('lumafield'),
    ('lumapictures'),
    ('lumary'),
    ('lumin'),
    ('luminai'),
    ('luminary-labs'),
    ('luminarycloud'),
    ('lumindigital'),
    ('luminostics'),
    ('lumiq-learn'),
    ('lumivero'),
    ('lumosity'),
    ('lumotive'),
    ('lunaphysicaltherapy'),
    ('lunchboxtechnologies'),
    ('luni'),
    ('luumlash'),
    ('luxe'),
    ('luxortechnology'),
    ('luxurypresence'),
    ('luzia'),
    ('lvs1'),
    ('lwolf'),
    ('lyciatherapeutics'),
    ('lykospbc'),
    ('lyra'),
    ('lyraclinical'),
    ('lyracollective'),
    ('lyrahealth'),
    ('lyrebirdstudio'),
    ('lyterian'),
    ('m-kopa'),
    ('m13'),
    ('m33'),
    ('mabl'),
    ('mable'),
    ('macandmia'),
    ('machinalabs'),
    ('machinefilab'),
    ('machinerypartner'),
    ('macquarietechnologygroup'),
    ('mactores'),
    ('madbox'),
    ('madhappy'),
    ('madhive'),
    ('madisonei'),
    ('madisonlogic'),
    ('madrigalpharma'),
    ('magic'),
    ('magicalbeginningslc'),
    ('magicgames'),
    ('magnetforensics'),
    ('magnify'),
    ('magnitudesoftware'),
    ('magnopus'),
    ('mahanatherapeutics'),
    ('mahmee'),
    ('mainspringenergy'),
    ('mainstreet'),
    ('mainstreetwork'),
    ('make-rain'),
    ('makersplace'),
    ('makeschool'),
    ('makpar'),
    ('malbongolf'),
    ('malt'),
    ('mamamoney'),
    ('mammothbiosci'),
    ('mamo'),
    ('mangolanguage'),
    ('manifest'),
    ('manifestfinancial'),
    ('manifestmedex'),
    ('mantanetwork'),
    ('mantisinnovation'),
    ('manty'),
    ('maple-finance'),
    ('maplemedia'),
    ('mapsbcorp'),
    ('maquettevirtuelle'),
    ('marcopolo'),
    ('marcusgrahamproject'),
    ('marcusmillichap'),
    ('margo-group'),
    ('margueritecaseyfoundation'),
    ('maribelhealth'),
    ('marigoldhealth'),
    ('markato'),
    ('marketcircle'),
    ('marketer-hire'),
    ('marketingevolution'),
    ('marketopia'),
    ('marlettefunding'),
    ('marq'),
    ('marqvision'),
    ('marshallreddick'),
    ('martinagency'),
    ('mashgin'),
    ('massive-rocket'),
    ('massschoolbuildings'),
    ('masterborn-2'),
    ('masterycharter'),
    ('mastreforestation'),
    ('mastrics'),
    ('masygroup'),
    ('matchgroup'),
    ('mathpresso'),
    ('matillion'),
    ('matrixpartners'),
    ('mattermark'),
    ('mattermost'),
    ('matterport'),
    ('matters'),
    ('matterworks'),
    ('max-retail'),
    ('maxmind'),
    ('mbcapital'),
    ('mbrdna'),
    ('mbta'),
    ('mcaconnect'),
    ('mcchrystalgroup'),
    ('mcgovern'),
    ('mcgrathnicol'),
    ('mcra'),
    ('mechanicalorchard'),
    ('mechanism-2'),
    ('medal'),
    ('medallia'),
    ('medcarepediatric'),
    ('medchart'),
    ('medely'),
    ('mediafly'),
    ('mediagenix'),
    ('medialab'),
    ('mediaocean'),
    ('medium'),
    ('mednow'),
    ('medrhythms'),
    ('meds'),
    ('meesho'),
    ('meetalbert'),
    ('meetgroup'),
    ('meetingstreetschools'),
    ('meetingtomorrow'),
    ('megaport'),
    ('meili'),
    ('meliuz'),
    ('membersy'),
    ('memebox'),
    ('memfault'),
    ('memphismeats'),
    ('mendix'),
    ('menlovc'),
    ('menta'),
    ('mentormate'),
    ('meridianlink'),
    ('merit'),
    ('merkle'),
    ('merklescience'),
    ('merlinlabs'),
    ('meroxa'),
    ('meshcapade'),
    ('messari'),
    ('metabase'),
    ('metagood'),
    ('metalpay'),
    ('metaprisebanking'),
    ('metasite'),
    ('metatheory'),
    ('metawealth'),
    ('meteor'),
    ('metergysolutions'),
    ('metlife'),
    ('metopera'),
    ('metr'),
    ('mgac'),
    ('mheducation'),
    ('mhnchicago'),
    ('micromerchantsystems'),
    ('microventures'),
    ('microvision'),
    ('midigator'),
    ('mighty'),
    ('migo-2'),
    ('miinto'),
    ('milk-moovement'),
    ('milliondollarbabyco'),
    ('milltownpartners'),
    ('milocredit'),
    ('mindbloom'),
    ('mindsdb'),
    ('mindtickle'),
    ('mineral'),
    ('minesense'),
    ('minimumltd'),
    ('minted'),
    ('minthouse'),
    ('mirai-foods'),
    ('mirego'),
    ('mirror'),
    ('missionbarns'),
    ('missioncloud'),
    ('missioncontinues'),
    ('missiongraduates'),
    ('missionhh'),
    ('missionwired'),
    ('mistplay'),
    ('mistral'),
    ('mit'),
    ('miteksystems-2'),
    ('mittilabs'),
    ('mixlab'),
    ('mixtiles'),
    ('mobile-action'),
    ('mobileintegration-group'),
    ('mobisystems'),
    ('modeln'),
    ('modern'),
    ('modern-age'),
    ('modernanimal'),
    ('modernintelligence'),
    ('modernlife'),
    ('modjo'),
    ('modsquad'),
    ('modsy'),
    ('modularps'),
    ('modulate'),
    ('moego'),
    ('mojo-video'),
    ('molcure'),
    ('molekule'),
    ('moltenindustries'),
    ('moment-ranks'),
    ('momentfactory'),
    ('momenti-inc'),
    ('momentum-inc'),
    ('momsmeals'),
    ('moneyboxapp'),
    ('moneytree'),
    ('monumental'),
    ('moo'),
    ('moogsoft'),
    ('moonhub'),
    ('moonpay'),
    ('moonpig'),
    ('moonsong-labs'),
    ('morningbrew'),
    ('morningconsult'),
    ('mosaic-2'),
    ('mosaicbuilders'),
    ('mostly-ai'),
    ('mostudio'),
    ('mothernewyork'),
    ('motifsystems'),
    ('motional'),
    ('motionapp'),
    ('motivateco'),
    ('motive'),
    ('mountaintop'),
    ('mous'),
    ('moveloot'),
    ('moxionpower'),
    ('mrss'),
    ('mryum'),
    ('mti-inc'),
    ('mtspartners'),
    ('mujininc'),
    ('mulliganfunding'),
    ('multiplylabs'),
    ('multiversx'),
    ('munichelectrification'),
    ('murgadoautomotive'),
    ('musixmatch'),
    ('muttdata'),
    ('myassurehealth'),
    ('mycelium'),
    ('myob-2'),
    ('myodetox'),
    ('myollie'),
    ('mypassglobal'),
    ('myplacehealth'),
    ('mytos'),
    ('mytraffic'),
    ('myvessel'),
    ('myvest'),
    ('nabis'),
    ('nahc'),
    ('nainnorcal'),
    ('narmi'),
    ('nasafcu'),
    ('natronenergy'),
    ('naturesfynd'),
    ('nautical-commerce'),
    ('nava'),
    ('navisite'),
    ('nce'),
    ('nclusioninc'),
    ('nebra-labs'),
    ('nectarpr'),
    ('neednova'),
    ('neighbor'),
    ('nekohealth'),
    ('nelly'),
    ('nemesiscorporation'),
    ('neo4j'),
    ('neon'),
    ('neonexchange'),
    ('neowiz'),
    ('nesthealth'),
    ('nestig'),
    ('nesto'),
    ('nestwealth'),
    ('netflix'),
    ('netlight'),
    ('netomi'),
    ('netvirta'),
    ('neuralink'),
    ('neurolabs-ai'),
    ('neurologica'),
    ('neuron7'),
    ('nevados'),
    ('newculturefood'),
    ('newfrontinsurance'),
    ('newton'),
    ('newton-school'),
    ('newwestern'),
    ('newwesterncorporate'),
    ('newzealandtradeandenterprise'),
    ('nex'),
    ('nexient'),
    ('nexigendigital'),
    ('nexos'),
    ('next-decade'),
    ('next-health'),
    ('nextbigsound'),
    ('nextcaller'),
    ('nextdata'),
    ('nextech'),
    ('nextgames'),
    ('nextmillennium'),
    ('nexuse-group'),
    ('nfamilyclub-2'),
    ('nfq'),
    ('nftnow'),
    ('nftpay'),
    ('nfx'),
    ('niagenbioscience'),
    ('nibiru'),
    ('nielsen'),
    ('nightfall'),
    ('nilc'),
    ('nimbleai'),
    ('nimblerx'),
    ('nimbletalent'),
    ('nimbus'),
    ('ninjavan'),
    ('nio'),
    ('nisum'),
    ('nithio'),
    ('nitra'),
    ('nium'),
    ('njstateofficeofinnovation'),
    ('noblereach'),
    ('nobullproject'),
    ('noibutechnologies'),
    ('nolapublicschools'),
    ('nomagic'),
    ('nomihealth'),
    ('nominal'),
    ('noodle'),
    ('norcalcann'),
    ('nordsec'),
    ('normai'),
    ('normalyze'),
    ('northatlanticcorp'),
    ('northernlabs'),
    ('notabene'),
    ('notablelabs'),
    ('notarize'),
    ('notch'),
    ('noteworth'),
    ('notonthehighstreet'),
    ('novaspace'),
    ('novatalent'),
    ('novi'),
    ('novir'),
    ('novosfiber'),
    ('noyo'),
    ('npowermedicine'),
    ('nrccua'),
    ('nrwl'),
    ('nsin'),
    ('ntdsoftware'),
    ('ntropy-network'),
    ('nubrakes'),
    ('numeral'),
    ('numeris'),
    ('nuorder'),
    ('nurx'),
    ('nutcrackertherapeutics'),
    ('nuuds'),
    ('nuvoair'),
    ('nveloptherapeutics'),
    ('nxcr'),
    ('nxscale'),
    ('nylas'),
    ('nymedia'),
    ('nyscf-2'),
    ('o-p-e-n'),
    ('oatly'),
    ('obie'),
    ('objective'),
    ('obol-tech'),
    ('observablehq'),
    ('observeai'),
    ('observeinc'),
    ('obsglobal'),
    ('ochre-bio'),
    ('oci'),
    ('ockam'),
    ('ocrolus'),
    ('octane'),
    ('octoenergy'),
    ('octoml'),
    ('octopus'),
    ('oda'),
    ('odaseva'),
    ('oddin'),
    ('odysaviation'),
    ('offchainlabs'),
    ('offerpop'),
    ('ogury'),
    ('okendo'),
    ('oktopost'),
    ('oleria-security'),
    ('olo'),
    ('omni-network'),
    ('omnidesigntech'),
    ('omnidian'),
    ('omnisend'),
    ('onaroll'),
    ('onceuponafarm'),
    ('ondeck'),
    ('ondofinance'),
    ('oneconcern'),
    ('onefinance'),
    ('onefootball'),
    ('onehouse'),
    ('oneimpression'),
    ('onesignal'),
    ('oneteam'),
    ('onfido'),
    ('ontic'),
    ('onuniverse'),
    ('onupkeep'),
    ('oomnitza'),
    ('oowlish'),
    ('opala'),
    ('opaque-systems'),
    ('opcity'),
    ('openai'),
    ('opencare'),
    ('opendoor'),
    ('openfarminc'),
    ('opengov'),
    ('openinvest'),
    ('openlogix'),
    ('openpayd'),
    ('openphone'),
    ('opensauced'),
    ('opensea'),
    ('openstore'),
    ('openteam'),
    ('openx'),
    ('operator'),
    ('ophelia'),
    ('opinov8'),
    ('optimism'),
    ('optimusride'),
    ('optionmetrics'),
    ('optiv-2'),
    ('opus1'),
    ('orbaerospace'),
    ('orbillion'),
    ('orbitalinsight'),
    ('orbitalsidekick'),
    ('orcabiosystems'),
    ('orcamobility'),
    ('orijin'),
    ('orkes'),
    ('orum'),
    ('osaro'),
    ('osedea'),
    ('osmind'),
    ('osnational'),
    ('oso'),
    ('osomprivacy'),
    ('otilumionics'),
    ('ou-education-services'),
    ('our-place'),
    ('ouster'),
    ('outlast'),
    ('outpacebio'),
    ('outreach'),
    ('outrider'),
    ('outschool'),
    ('outsight'),
    ('outsourcedstaff'),
    ('overbit'),
    ('overbond'),
    ('overmind'),
    ('overmoon'),
    ('owner'),
    ('oxeonpartners'),
    ('oxio'),
    ('oxman'),
    ('oxylabs'),
    ('p-1ai'),
    ('p3vetpartners'),
    ('pachama'),
    ('pachyderm'),
    ('pacific-environment'),
    ('pacificoenergy'),
    ('packagex'),
    ('packmatic'),
    ('padsplit'),
    ('pagerduty'),
    ('palantir'),
    ('pamu'),
    ('panapartners'),
    ('panda'),
    ('panderasystems'),
    ('pano'),
    ('panopto'),
    ('pansns'),
    ('panteracapital'),
    ('panum'),
    ('papara'),
    ('papercut'),
    ('paradoxum-gg'),
    ('paralleldomain'),
    ('parallelwireless'),
    ('paramedicservices'),
    ('parcellab'),
    ('pardon'),
    ('parkplace'),
    ('parnalllaw'),
    ('parrotsa'),
    ('parsleyhealth'),
    ('parspec'),
    ('participantmedia'),
    ('partiful'),
    ('partoo'),
    ('partyslate'),
    ('passport'),
    ('passportshipping'),
    ('patch'),
    ('patchmypc'),
    ('pathpoint'),
    ('pathrise'),
    ('patientcolife'),
    ('patsnap'),
    ('pattern'),
    ('pavementcoffee'),
    ('pax8'),
    ('payabli'),
    ('payactiv'),
    ('payfit'),
    ('payjoy'),
    ('paymongo'),
    ('paystack'),
    ('paytm'),
    ('paytmbank'),
    ('paytmpayments'),
    ('payugpo'),
    ('payzen'),
    ('pditechnologies'),
    ('pdq'),
    ('peachfinance'),
    ('peakdentalservices'),
    ('peakdesign'),
    ('peakgames'),
    ('peakmetrics'),
    ('peckwaterbrands'),
    ('peddie'),
    ('peerspace'),
    ('pelago'),
    ('pelmorex'),
    ('pendulum'),
    ('pennylane'),
    ('pentagrp'),
    ('pento'),
    ('penumbrainc'),
    ('people-ai'),
    ('peoplefun'),
    ('peoplegrove'),
    ('percipient'),
    ('percona'),
    ('perennial'),
    ('perforce'),
    ('perrknight'),
    ('persistiq'),
    ('persona'),
    ('pestotech'),
    ('petalcard'),
    ('petcircle'),
    ('peteforamerica'),
    ('petfriendly'),
    ('petvisor'),
    ('pex'),
    ('pexa'),
    ('phaze-technologies'),
    ('phiatech'),
    ('phoenixtailings'),
    ('photoroom'),
    ('phrasee'),
    ('pibenchmark'),
    ('piccollage'),
    ('pickethomes'),
    ('picklerobot'),
    ('picsart'),
    ('picus'),
    ('pieinsurance'),
    ('piepacker'),
    ('pigment'),
    ('pillarbiosciences'),
    ('pillow'),
    ('pinata'),
    ('pinegames'),
    ('pineparkhealth'),
    ('pingcap'),
    ('pingwind'),
    ('pinkoi'),
    ('pioneer-services'),
    ('pip'),
    ('pipedrive'),
    ('piplabs'),
    ('pirouettemedical'),
    ('pison'),
    ('pivotal'),
    ('pivotenergy'),
    ('pjp'),
    ('placemakr'),
    ('placemeter'),
    ('placepass'),
    ('plaid'),
    ('plailabsinc'),
    ('planetinnovation'),
    ('planettechnologies'),
    ('plangrid'),
    ('planned'),
    ('planned-2'),
    ('planner5d'),
    ('plastiq'),
    ('platform-stud'),
    ('playco'),
    ('playonsports'),
    ('playplay'),
    ('playvs'),
    ('plazahomemortgage'),
    ('plenti'),
    ('plexus'),
    ('plivo'),
    ('plooto'),
    ('plum'),
    ('plume'),
    ('plus-2'),
    ('plusgrade'),
    ('pmaconsultants'),
    ('pmdsoft'),
    ('pockethealth'),
    ('pocketworlds'),
    ('pointb'),
    ('pointclickcare'),
    ('pointonenav'),
    ('poki'),
    ('polarsteps'),
    ('policyfly'),
    ('policyme'),
    ('polleverywhere'),
    ('pollyex'),
    ('polygon'),
    ('polymerlabs'),
    ('polywork'),
    ('pond5'),
    ('poplarhomes'),
    ('popmenu'),
    ('popsa'),
    ('popsockets'),
    ('portagepointpartners'),
    ('portcast'),
    ('porter'),
    ('portoro'),
    ('portpro'),
    ('poshtech'),
    ('possiblefinance'),
    ('postera'),
    ('potentialmotors'),
    ('potloc'),
    ('powdevs'),
    ('pp-la'),
    ('ppaz'),
    ('ppcentralcoast'),
    ('ppfa'),
    ('ppgnnj'),
    ('ppgreatplains'),
    ('ppgt'),
    ('ppil'),
    ('ppkeystone'),
    ('pplm'),
    ('pplwise'),
    ('ppmd'),
    ('ppmi'),
    ('ppmontana'),
    ('ppncny'),
    ('ppnne'),
    ('ppnyc'),
    ('pppsgv'),
    ('pprm'),
    ('ppsat'),
    ('ppsenfl'),
    ('ppslrswmo'),
    ('ppsne'),
    ('ppsp'),
    ('ppswo'),
    ('ppwi'),
    ('practicebetter'),
    ('precede'),
    ('prefixbox'),
    ('preligens'),
    ('prelim'),
    ('premia'),
    ('premiertruck'),
    ('prescriptivedata'),
    ('prestashop'),
    ('prestigedevelopment'),
    ('presto'),
    ('prestolabs'),
    ('pretto'),
    ('prevail'),
    ('pricebenowitz'),
    ('pricemoov'),
    ('prilenia'),
    ('primary-2'),
    ('primarybio'),
    ('primaryvc'),
    ('primechiefofstaff'),
    ('primee'),
    ('primeexecutiveoffice'),
    ('primemoverslab'),
    ('princesspolly'),
    ('princeton10'),
    ('printavo'),
    ('printify'),
    ('priorities'),
    ('prismic'),
    ('privacy'),
    ('prizeout'),
    ('procept-biorobotics'),
    ('processunity'),
    ('proconconsulting'),
    ('procreate'),
    ('prodigyeducation'),
    ('prodigygame'),
    ('productable'),
    ('productiv'),
    ('productventures'),
    ('profoundresearch'),
    ('progressive-design'),
    ('project-healthy-minds'),
    ('projectcallisto'),
    ('projector-2'),
    ('projectyx'),
    ('prolific-machines'),
    ('promaxo'),
    ('promenade'),
    ('promesaacademy'),
    ('prometheanworld'),
    ('prominentedge'),
    ('promiserobotics'),
    ('pronto-ai'),
    ('proof'),
    ('propellerhead'),
    ('propelplm'),
    ('propelsoftware'),
    ('proper'),
    ('prophethomes'),
    ('prosper'),
    ('protectenvironmental'),
    ('protective'),
    ('protegrity'),
    ('protocol'),
    ('protolabs'),
    ('protrials'),
    ('provectus'),
    ('provi'),
    ('proxima'),
    ('proximafusion'),
    ('pryon'),
    ('psintegrated'),
    ('psivant'),
    ('pspdfkit'),
    ('pt78'),
    ('publiccloudgroup-2'),
    ('publicprep'),
    ('pubstack'),
    ('pulsedata'),
    ('pulsemedianl'),
    ('pumpkin'),
    ('purchasingplatform'),
    ('puritanmedproducts'),
    ('purple-technology'),
    ('purplelandmgmt'),
    ('pushpress'),
    ('pushsecurity'),
    ('pyramidci'),
    ('pythian'),
    ('q-centrix'),
    ('q-ctrl'),
    ('q4inc'),
    ('qarik'),
    ('qashier'),
    ('qbio'),
    ('qcells'),
    ('qcells-2'),
    ('qloo'),
    ('qogita'),
    ('qonto'),
    ('qover'),
    ('qredo'),
    ('qrypt'),
    ('qsic'),
    ('qualdoc'),
    ('qualified'),
    ('qualtekservices'),
    ('qualysoft'),
    ('quandri'),
    ('quantcast'),
    ('quantco-'),
    ('quantgene'),
    ('quantlabs'),
    ('quantumcircuits'),
    ('quantummetric'),
    ('quantumworkplace'),
    ('quartet'),
    ('quartethealth'),
    ('quartzy'),
    ('quenchwater'),
    ('questanalytics'),
    ('quicknode'),
    ('quidd'),
    ('quidel'),
    ('quilt'),
    ('quilted-health'),
    ('quince'),
    ('quincus'),
    ('quincyinst'),
    ('quitgenius'),
    ('quizizz'),
    ('quizlet-2'),
    ('quokka'),
    ('quora'),
    ('quotapath'),
    ('quotevelocity'),
    ('quotient'),
    ('quri'),
    ('r4d'),
    ('rabbitholestudios'),
    ('rackspace'),
    ('radai'),
    ('radformation'),
    ('radicalai'),
    ('radix'),
    ('radpowerbikes-2'),
    ('rai'),
    ('railroad19'),
    ('raine'),
    ('rainfocus'),
    ('rainforest'),
    ('rainmaking'),
    ('raise'),
    ('ramenvr'),
    ('rangeenergy'),
    ('ranger'),
    ('ranitherapeutics'),
    ('rapidai'),
    ('rapidapi'),
    ('rapiddeploy'),
    ('rapidmicrobio'),
    ('rappi'),
    ('raptormaps'),
    ('raptstudio'),
    ('raptv'),
    ('rarible'),
    ('ratemyagent'),
    ('raven'),
    ('raya'),
    ('rbanw'),
    ('reachpower'),
    ('reali'),
    ('realscout'),
    ('realvision'),
    ('realworklabs'),
    ('recast-software'),
    ('recess'),
    ('recognisebank'),
    ('recordpoint'),
    ('recormedical'),
    ('recur'),
    ('recurly'),
    ('recurrency'),
    ('recurve'),
    ('redaptiveinc'),
    ('redbee'),
    ('redcanary'),
    ('redcanyonsoftware'),
    ('reddit'),
    ('reddogmedia'),
    ('redhorsecorp'),
    ('redoxengine'),
    ('redsox'),
    ('redwoodcu'),
    ('redwoodresearch'),
    ('redwoodstrategygroup'),
    ('reebelo'),
    ('reekon-tools'),
    ('refed'),
    ('reflectorentertainment'),
    ('refuel'),
    ('regalvoice'),
    ('regard'),
    ('regentcraft'),
    ('regenxbio'),
    ('reggora'),
    ('regrello'),
    ('reibus'),
    ('reifyhealth'),
    ('relativity'),
    ('relay'),
    ('relay-commerce'),
    ('relayfi'),
    ('relayr'),
    ('relaytx'),
    ('reli-sh'),
    ('reliable'),
    ('remedyentertainment'),
    ('remedyproductstudio'),
    ('remine'),
    ('remix'),
    ('remofirst'),
    ('rendernetworks'),
    ('renegade'),
    ('renofi'),
    ('repairsmith'),
    ('replicant'),
    ('reply'),
    ('repreach'),
    ('republic'),
    ('repurposeglobal'),
    ('reputation'),
    ('request'),
    ('rescale'),
    ('researchgate'),
    ('residenthome'),
    ('resilience'),
    ('resiliencelab'),
    ('resilientco'),
    ('resilinc'),
    ('resonanceglobal'),
    ('respawn'),
    ('respondent'),
    ('restaurant365'),
    ('restream'),
    ('resultadosdigitais'),
    ('retailmenot'),
    ('retailreinvented'),
    ('retentionscience'),
    ('retool'),
    ('retro'),
    ('returnly'),
    ('returntocorp'),
    ('revealtech'),
    ('revefi'),
    ('revenueanalytics'),
    ('revenuecat'),
    ('revenuemanage'),
    ('reverielabs'),
    ('revero'),
    ('revhealth'),
    ('review'),
    ('reviewtrackers'),
    ('revinate'),
    ('reviverx'),
    ('revolut'),
    ('revolutionparts'),
    ('revzilla'),
    ('rewatch'),
    ('rezolutebio'),
    ('rezotx'),
    ('rfa-2'),
    ('rgi'),
    ('rhombus-systems'),
    ('richards-supply'),
    ('rideonewheel'),
    ('rideos'),
    ('ridereport'),
    ('ridezum'),
    ('ridwell'),
    ('riftfinance'),
    ('rigetti'),
    ('rightformula'),
    ('rightsideup'),
    ('riley'),
    ('rilldata'),
    ('rillet'),
    ('rimsys'),
    ('ring'),
    ('riocan'),
    ('riptidetech'),
    ('rise'),
    ('risklabs'),
    ('ritual'),
    ('ritual-technologies'),
    ('riverdale'),
    ('riveron'),
    ('rivosinc'),
    ('rivr'),
    ('rld-foundation'),
    ('ro'),
    ('roadio-ai'),
    ('roadrunner-studios'),
    ('roadrunnerwm'),
    ('robinpowered'),
    ('roboflow'),
    ('roboticassistancedevices'),
    ('robust-ai'),
    ('robustintelligence'),
    ('rockcontent'),
    ('rocketcommunications'),
    ('rocketlawyer'),
    ('rocketpartners'),
    ('rocketpharma'),
    ('rocketrip'),
    ('rocketship'),
    ('rockset'),
    ('rockymountainprep'),
    ('roger'),
    ('roh'),
    ('roico'),
    ('rollbar'),
    ('roo'),
    ('roofr'),
    ('roofstacks'),
    ('roofstock'),
    ('rookcoffee'),
    ('roosterteeth'),
    ('root16'),
    ('roserocket'),
    ('roshalimaging'),
    ('roundglass'),
    ('routethis'),
    ('rover'),
    ('rovio-2'),
    ('roy-cooper'),
    ('royal'),
    ('royalambulance'),
    ('rqteam'),
    ('rsandf-2'),
    ('ruangguru'),
    ('rubylabs'),
    ('rumin8'),
    ('run-alloy'),
    ('runforsomething'),
    ('runkeeper'),
    ('runningtide'),
    ('rupa'),
    ('rushdownstudio'),
    ('rw1'),
    ('rws'),
    ('rxsavingssolutions-2'),
    ('rylo'),
    ('ryzlabs'),
    ('saagie'),
    ('safe'),
    ('safeai'),
    ('safelease'),
    ('safetyculture-2'),
    ('safran-ai'),
    ('saga-xyz'),
    ('sagana'),
    ('sage-growth'),
    ('sagesure'),
    ('saiglobal'),
    ('sait'),
    ('sakuu'),
    ('saleor'),
    ('salesmsg'),
    ('saleswhale'),
    ('salo-sciences'),
    ('salvohealth'),
    ('samacare'),
    ('sambatv'),
    ('sanabenefits'),
    ('sanctuary'),
    ('sandboxvr'),
    ('sar'),
    ('saronic'),
    ('sarwa'),
    ('satoricapital'),
    ('sauce'),
    ('savage-interactive'),
    ('savii-2'),
    ('saviynt'),
    ('sayari'),
    ('sayurbox'),
    ('scalablepress'),
    ('scalac'),
    ('scale3c'),
    ('scaleai'),
    ('scalebio'),
    ('scalemicrogridsolutions'),
    ('scaleway'),
    ('scality'),
    ('scanifly'),
    ('scanlinevfx'),
    ('schibsted'),
    ('schmidt-entities'),
    ('scholarrock'),
    ('scholarsoffinance'),
    ('schoolhouseworld'),
    ('scienceexchange'),
    ('scio'),
    ('scoopforwork'),
    ('scopear'),
    ('scoperecruiting'),
    ('scorpion'),
    ('scottlogic'),
    ('scottscheapflights'),
    ('scrapinghub'),
    ('scribd'),
    ('scrollfoundation'),
    ('sdhrconsulting'),
    ('seaoil'),
    ('seaportglobal'),
    ('secondfrontsystems'),
    ('section'),
    ('securecodewarrior'),
    ('secureframe'),
    ('securityinnovation'),
    ('seedify-fund'),
    ('seekai'),
    ('seerinteractive'),
    ('seesaw'),
    ('seifdn'),
    ('seilabs'),
    ('selfbook'),
    ('selfmade'),
    ('semanticbits'),
    ('semaphoresolutions'),
    ('semgrep'),
    ('sendoso'),
    ('sensefinance'),
    ('sensible'),
    ('sensortower'),
    ('sentibio'),
    ('seoulrobotics'),
    ('sep'),
    ('septerna'),
    ('seqera'),
    ('seranbio'),
    ('serigraph'),
    ('serotonin'),
    ('serv-u-success'),
    ('servicerocket'),
    ('sesame'),
    ('seven-starling'),
    ('sevenbridges'),
    ('sewerai'),
    ('sfcg'),
    ('sfeir'),
    ('sfgiants'),
    ('sfmoma'),
    ('shadow'),
    ('shakacode'),
    ('shakepay'),
    ('shakudo'),
    ('shapertools'),
    ('sharethrough'),
    ('sheerid'),
    ('sheertex'),
    ('shef'),
    ('shelfengine'),
    ('shieldai'),
    ('shippit'),
    ('shippo'),
    ('shiru'),
    ('shop-canal'),
    ('shopback-2'),
    ('shopify'),
    ('shopreli'),
    ('showcaseidx'),
    ('showpo-2'),
    ('shyftlabs'),
    ('sidebench'),
    ('sideinc'),
    ('sidequest'),
    ('siepe'),
    ('sierraclub'),
    ('sietefoods-2'),
    ('sigfig-2'),
    ('sightviewsoftware'),
    ('signal'),
    ('signalfx'),
    ('significanceinc'),
    ('signinsolutions'),
    ('sila'),
    ('silasg'),
    ('silo'),
    ('silverbackstrategies'),
    ('silvercar'),
    ('silviaterra'),
    ('silvustechnologies'),
    ('simberobotics'),
    ('simplebet'),
    ('simplehabit'),
    ('simpleventures'),
    ('simpli-2'),
    ('simplybusiness'),
    ('simulmedia'),
    ('singerlewak'),
    ('siren-bio'),
    ('sirnaomics'),
    ('siteground'),
    ('siteminder'),
    ('sitetracker'),
    ('sitka'),
    ('sixfoldbioscience'),
    ('sixspeed'),
    ('skava'),
    ('skello'),
    ('sketch'),
    ('skill-lync'),
    ('skillerszone'),
    ('skilljar'),
    ('skillshare'),
    ('skims'),
    ('skio'),
    ('skipscooters'),
    ('skulabs'),
    ('skyboxlabs'),
    ('skydance'),
    ('skygrid'),
    ('skylineconstruction'),
    ('skypointcloud'),
    ('skyralio'),
    ('skyslope'),
    ('skywarditsolutions'),
    ('skyways'),
    ('slab'),
    ('slang'),
    ('slangai'),
    ('slate'),
    ('sleuth'),
    ('smarking'),
    ('smarsh'),
    ('smart-working-solutions'),
    ('smartadserver'),
    ('smartbugoperatingllc'),
    ('smartcar'),
    ('smartenergy'),
    ('smartthings'),
    ('smiledigitalhealth'),
    ('snacknation'),
    ('snackpass'),
    ('snapcommerce'),
    ('snapdocs'),
    ('snaplogic'),
    ('snappr'),
    ('snif'),
    ('snowfork'),
    ('snowplow'),
    ('snowplowanalytics'),
    ('socialchorus'),
    ('socialtables'),
    ('sofarsounds'),
    ('softrams'),
    ('softworld'),
    ('sogoodsoyou'),
    ('solarlandscape'),
    ('solestial'),
    ('solfl'),
    ('solidcore'),
    ('soloavt'),
    ('solopulseco'),
    ('solutiondesign'),
    ('solutionsjournalism'),
    ('solutuslegal'),
    ('solvd'),
    ('solvhealth'),
    ('sonar'),
    ('sonarsource'),
    ('sonatus'),
    ('sonatype'),
    ('soniresources'),
    ('sonrai-security'),
    ('sophos'),
    ('soraschools'),
    ('soundcommerce'),
    ('soundstack'),
    ('source'),
    ('sourcegraph'),
    ('southwestwater'),
    ('soylent'),
    ('spaceback'),
    ('spacemaker'),
    ('spacetecpartners'),
    ('sparkclimatesolutions'),
    ('sparkfund'),
    ('spawglass'),
    ('speakeasyapi'),
    ('spear'),
    ('spear-ai'),
    ('spectral'),
    ('spekit'),
    ('spendesk'),
    ('sphinxdefense'),
    ('spiceai'),
    ('spinai'),
    ('spine-media'),
    ('spinwheel'),
    ('spiria'),
    ('splash-music'),
    ('splashfinancial'),
    ('splend'),
    ('spmb'),
    ('sporty'),
    ('spotify'),
    ('spreedly'),
    ('spreetail'),
    ('springboard'),
    ('springfreeev'),
    ('springhealth'),
    ('sprinterhealth'),
    ('sprintfwd'),
    ('sprinto'),
    ('spruce'),
    ('sprucestreetcomp'),
    ('sprucestreetcompinternships'),
    ('sprucesystems'),
    ('sprymethods'),
    ('sprypointservices'),
    ('spyke-games'),
    ('sqaservices'),
    ('sqreen-2'),
    ('sta'),
    ('stable'),
    ('stackadapt'),
    ('stackblitz'),
    ('stackedsp'),
    ('stackhawk'),
    ('stacksfoundation'),
    ('standtogether'),
    ('starburstdata'),
    ('starcompliance'),
    ('stardog'),
    ('starkbank'),
    ('starknet'),
    ('starship'),
    ('startengine'),
    ('startuplifers'),
    ('startuptap'),
    ('statesproject'),
    ('statflo'),
    ('statuspageio'),
    ('statuspro'),
    ('stavvy'),
    ('stedi'),
    ('steemit'),
    ('stellar'),
    ('stellarfinance'),
    ('stem'),
    ('stemprepacademy'),
    ('stevenstransport'),
    ('stickermule'),
    ('stimlabs'),
    ('stockpile'),
    ('stoic'),
    ('stoneridgesoftware'),
    ('storiogroup'),
    ('stormforge'),
    ('storygrounds'),
    ('storyprotocol'),
    ('stradaeducation'),
    ('straighterline'),
    ('strapi'),
    ('streetcontext'),
    ('streetlightdata'),
    ('streetsmartsvr'),
    ('stridefunding'),
    ('strongcompute'),
    ('strongdm'),
    ('stronghold'),
    ('structionsite'),
    ('strydefurther'),
    ('stuart'),
    ('stubhub'),
    ('studiothree'),
    ('studypoint'),
    ('stxnft'),
    ('sublimesystems'),
    ('subspacelabs'),
    ('substackinc'),
    ('sugarcrm'),
    ('suger'),
    ('sukut'),
    ('summersalt'),
    ('sumo-digital'),
    ('sundaysfordogs'),
    ('sundrivesolar'),
    ('sunnova'),
    ('sunpower'),
    ('sunrisefarms'),
    ('sunshinesachs'),
    ('sunsrce'),
    ('suntribegroup'),
    ('sunwatercapital'),
    ('super'),
    ('super-com'),
    ('superannotate'),
    ('superiorexecutiveandlegalrecruiting'),
    ('supermove'),
    ('supernovacompanies'),
    ('supernoya-logistics'),
    ('superpedestrian'),
    ('superside'),
    ('superstate'),
    ('supportkind'),
    ('supportninja'),
    ('sure'),
    ('surfshark'),
    ('surglogs'),
    ('sustain-cert'),
    ('swaniti'),
    ('swapcard'),
    ('swappie'),
    ('sweedpos'),
    ('swiftconnect'),
    ('swiftlysystems'),
    ('swiftmedical'),
    ('swiggy'),
    ('swile'),
    ('swimply'),
    ('swingdev'),
    ('swingvision'),
    ('swiss-mile'),
    ('swissblock'),
    ('swissborg'),
    ('switchboardsoftware'),
    ('swoopapp'),
    ('swordhealth'),
    ('swyftx'),
    ('sygnaturediscovery'),
    ('sylvera'),
    ('symmetryinvestments'),
    ('symmetrysystems'),
    ('symplicity'),
    ('synapseint'),
    ('synapticure'),
    ('synergyecp'),
    ('synmax'),
    ('synolotx'),
    ('syntax'),
    ('synthego'),
    ('synthetaic'),
    ('syntronic'),
    ('sysdig'),
    ('system1'),
    ('t1'),
    ('t6talentpartners'),
    ('tachyus'),
    ('tackle'),
    ('tact'),
    ('tactical'),
    ('tactilegames'),
    ('tag'),
    ('tagup'),
    ('tahoebio-ai'),
    ('tajir-app'),
    ('takescoop'),
    ('tala'),
    ('talend'),
    ('talent2win'),
    ('talentkompass-deutschland'),
    ('talentneuron'),
    ('talentus'),
    ('talkiatry'),
    ('talkwalker'),
    ('tallarium'),
    ('tamara'),
    ('tamr'),
    ('tandemcare'),
    ('tangibleplay'),
    ('tangraminteriors'),
    ('tapasmedia'),
    ('tapcart'),
    ('taplytics'),
    ('taptapsend'),
    ('tari'),
    ('tataunistore'),
    ('tavernresearch'),
    ('taxfyle'),
    ('tdaboulder'),
    ('tdthornton'),
    ('teachable'),
    ('teachinglab'),
    ('tealbook'),
    ('tealium'),
    ('teambuilders'),
    ('teameven'),
    ('teampilot'),
    ('teamshares'),
    ('teamsnap'),
    ('techflow'),
    ('techmatters'),
    ('technexus'),
    ('techquarter'),
    ('techruiter'),
    ('techtrust-inc'),
    ('teck-2'),
    ('tecla'),
    ('tecton'),
    ('tegus'),
    ('teikametrics'),
    ('teleo'),
    ('teleport'),
    ('telesat'),
    ('teller'),
    ('tempest'),
    ('temporal'),
    ('tenderly'),
    ('tendermint'),
    ('tendo'),
    ('tensorwave'),
    ('tenstorrent'),
    ('teramind'),
    ('terawattinfrastructure'),
    ('terazo'),
    ('terminus'),
    ('termius'),
    ('terms'),
    ('terra'),
    ('terraformation'),
    ('terrahq'),
    ('terrascend'),
    ('terrascope'),
    ('tesorio'),
    ('tessian'),
    ('tevora'),
    ('texas-original'),
    ('textio'),
    ('textnow'),
    ('tgsmc'),
    ('thalolabs'),
    ('the-sia-foundation'),
    ('the-street'),
    ('the-wing'),
    ('theambrgroup'),
    ('theathletic'),
    ('thebabyboxco'),
    ('theblacktux'),
    ('theblockcrypto'),
    ('thebrowser-company'),
    ('thecitizenry'),
    ('thecontigroup'),
    ('thedispatch'),
    ('theex'),
    ('theexteriorcompany'),
    ('theflexcompany'),
    ('thefp'),
    ('theguarantors'),
    ('thehousemajoritypac'),
    ('theinformationlab'),
    ('thejuggernaut'),
    ('thekey'),
    ('thekrazycouponlady'),
    ('themuse'),
    ('thenotcompany'),
    ('thentwrk'),
    ('thenudge'),
    ('theodo'),
    ('theorem'),
    ('theoremonellc'),
    ('theorycraftgames'),
    ('thepeoplebrand'),
    ('thepieexecsearch'),
    ('thepillclub'),
    ('therabodycorp'),
    ('therealdeal'),
    ('therecount'),
    ('therichardsgrp'),
    ('therounds'),
    ('thesalesfactory'),
    ('thesleepreset'),
    ('thespringhillco'),
    ('thestudio'),
    ('thetrainline'),
    ('thetrevorproject'),
    ('thewanderlustgroup'),
    ('thewire-media'),
    ('thezebra'),
    ('thiga'),
    ('thimble'),
    ('thinair'),
    ('thinkahead'),
    ('thinkcyte'),
    ('thinkful'),
    ('thinkhr'),
    ('thinkingbox'),
    ('thinkon'),
    ('thinkstermath'),
    ('third-city-christian-church'),
    ('third-plateau'),
    ('thirdlove'),
    ('thirdwavelabs'),
    ('thortech-solutions'),
    ('thoughtexchange'),
    ('thrasio'),
    ('threatconnect'),
    ('threattec'),
    ('thredup'),
    ('thrillworks'),
    ('thrive-therapies'),
    ('thrivecausemetics'),
    ('thrivemarket'),
    ('thuma'),
    ('thundertoken'),
    ('thunkable'),
    ('thycotic'),
    ('tiff'),
    ('tiket'),
    ('tilda-research'),
    ('tildei'),
    ('tileapp'),
    ('timely-md'),
    ('timelycare'),
    ('tinybird'),
    ('tiplink'),
    ('titan'),
    ('titanelectric-ga'),
    ('titanwh'),
    ('titanx'),
    ('titmouse'),
    ('tivix'),
    ('tkda'),
    ('tlnt'),
    ('todaytixgroup'),
    ('token'),
    ('tokenmetrics'),
    ('tokenwin'),
    ('toku'),
    ('tombras'),
    ('tomjames'),
    ('tomonetworks'),
    ('tonal'),
    ('tonic'),
    ('tonkean'),
    ('tonsser'),
    ('topanga'),
    ('topazlabs'),
    ('tophat'),
    ('tophatter'),
    ('topicalsrx'),
    ('topl'),
    ('toplinepro'),
    ('toptal'),
    ('torch'),
    ('torchdental'),
    ('toruslabs'),
    ('totalexpert'),
    ('totalsystech'),
    ('tovala'),
    ('tpgsi'),
    ('traackr'),
    ('traba'),
    ('trace3'),
    ('tracksuit-limited'),
    ('tracktik'),
    ('tractian'),
    ('trade-lanes'),
    ('tradecentriccareers'),
    ('trader'),
    ('tradeshift'),
    ('tradesy'),
    ('trailappliances'),
    ('trailofbits'),
    ('trainual'),
    ('tramgroup'),
    ('transfrvr'),
    ('transitscreen'),
    ('translifeline'),
    ('transmutex'),
    ('transwest'),
    ('travala'),
    ('travertine'),
    ('treasure-data'),
    ('treecard'),
    ('treering'),
    ('trela'),
    ('trella'),
    ('trellis'),
    ('trendyol'),
    ('trengobv'),
    ('trevipay'),
    ('tri'),
    ('triallibrary'),
    ('tribe-payments'),
    ('trility'),
    ('trilogyfederal'),
    ('trinityhunt'),
    ('trio'),
    ('trip-ninja'),
    ('tripalink'),
    ('tripleblind'),
    ('triplebyte'),
    ('triplewhale'),
    ('tripscout'),
    ('trivenibio'),
    ('trolley'),
    ('true-environmental'),
    ('trueaccord'),
    ('trueml'),
    ('truenorth'),
    ('truepill'),
    ('trueplatform'),
    ('truetandem'),
    ('truevault'),
    ('truezerotech'),
    ('trunkio'),
    ('truora'),
    ('trussworks'),
    ('trustandwill'),
    ('trustarc'),
    ('trustedhealth'),
    ('trustlab'),
    ('trustly'),
    ('trusttoken'),
    ('trustyou'),
    ('truv'),
    ('truveris'),
    ('trycourier'),
    ('tryjeeves'),
    ('tryriot'),
    ('tsmg'),
    ('ttecdigital'),
    ('ttxinc'),
    ('tubularlabs'),
    ('tuftandneedle'),
    ('tul'),
    ('tune'),
    ('tunein'),
    ('tunnl-data'),
    ('turing-labs'),
    ('turvo'),
    ('tuskventures'),
    ('tutorintelligence'),
    ('tweeq'),
    ('twelvelabs'),
    ('twentyfirstcenturybrand'),
    ('twingate'),
    ('twisp'),
    ('twitch'),
    ('txidigital'),
    ('txse'),
    ('typology'),
    ('uaustin'),
    ('uberflip'),
    ('ubiminds'),
    ('udacity'),
    ('udemy'),
    ('uei'),
    ('ueno'),
    ('ufonia'),
    ('uipath'),
    ('ultra'),
    ('ultrahealthproviders'),
    ('umaproject'),
    ('umbragestudios'),
    ('umpqua-health'),
    ('uncountable'),
    ('unico'),
    ('unify'),
    ('unifyconsulting'),
    ('uniphore'),
    ('unisoninfra'),
    ('unitedwayla'),
    ('unitq'),
    ('universalengineeringsciences'),
    ('unknownworlds'),
    ('unlearnai'),
    ('unlikely'),
    ('unlimit'),
    ('unraveldata'),
    ('unstoppabledomains'),
    ('unstructuredtechnologies'),
    ('untether'),
    ('unu'),
    ('unusual'),
    ('upgrade'),
    ('upguard'),
    ('uplift'),
    ('upmetrics'),
    ('uprightproject'),
    ('upside'),
    ('upstox'),
    ('upwork'),
    ('uqual'),
    ('usasurveyjob'),
    ('usecipher'),
    ('useinsider'),
    ('useloom'),
    ('userinterviews'),
    ('userlane'),
    ('ushur'),
    ('usmobile'),
    ('utopialabs'),
    ('utvate'),
    ('uvcyber'),
    ('v2sa'),
    ('vacancies'),
    ('vailsys'),
    ('valarian'),
    ('valdera'),
    ('valdeseng'),
    ('valence'),
    ('valiantys'),
    ('valinor'),
    ('valkyrietrading'),
    ('valoremreply'),
    ('valorepartners'),
    ('vana'),
    ('vannevarlabs-2'),
    ('vantagepointlogistics'),
    ('varicent'),
    ('varni-labs'),
    ('varomoney'),
    ('vauld'),
    ('vaxcyte'),
    ('vclab'),
    ('vectara'),
    ('vedadata'),
    ('veefriends'),
    ('veepee'),
    ('veeva'),
    ('veho'),
    ('veir'),
    ('velo3d'),
    ('vendavo'),
    ('venteur'),
    ('venusaero'),
    ('veo'),
    ('veonhq'),
    ('verafin'),
    ('verdigris'),
    ('vergesense'),
    ('vericred'),
    ('verifiable'),
    ('verisart'),
    ('verisinsights'),
    ('veritasinv'),
    ('verity'),
    ('verkada'),
    ('verkor'),
    ('vero'),
    ('versana'),
    ('versapay'),
    ('verse'),
    ('verse-medical'),
    ('verticalscope'),
    ('verygoodsecurity'),
    ('verypossible'),
    ('verytechnology'),
    ('vesta-tech'),
    ('vestahousing'),
    ('vestiairecollective'),
    ('vesync'),
    ('vetcove'),
    ('veteransadvantage'),
    ('vevo'),
    ('vevo-ai'),
    ('vhslab'),
    ('vhx'),
    ('viabill'),
    ('viacom18'),
    ('vial'),
    ('viamericas'),
    ('viantinc'),
    ('viaseparations'),
    ('vibrenthealth'),
    ('victoryhrg'),
    ('vida'),
    ('videahealth'),
    ('vidrovr'),
    ('vidsy'),
    ('viget'),
    ('vimpelcom'),
    ('vinted'),
    ('viome'),
    ('vipdesk'),
    ('vippetcare'),
    ('vir'),
    ('virtuous'),
    ('virtus'),
    ('visby'),
    ('viseven'),
    ('visier'),
    ('visioncritical'),
    ('visioninvest'),
    ('visotrust'),
    ('vistarmedia-2'),
    ('vital-software'),
    ('vitalbio'),
    ('vitalfarms'),
    ('vivacitylabs'),
    ('vivenu'),
    ('vivo-care'),
    ('vivrelle'),
    ('vivsoft'),
    ('vivun'),
    ('vizion'),
    ('vngrs'),
    ('vo2-group'),
    ('voleon'),
    ('volka'),
    ('voltacharging'),
    ('voltalabs'),
    ('voltus'),
    ('volume-finance'),
    ('voodoo'),
    ('voro'),
    ('voterformationproject'),
    ('voterparticipation'),
    ('vowel'),
    ('voxel'),
    ('voxy'),
    ('vrchat'),
    ('vrsystems'),
    ('vtex'),
    ('vts'),
    ('vvt'),
    ('waabi'),
    ('wachter'),
    ('wadetrim'),
    ('wahi'),
    ('waitrapp'),
    ('waitwhat'),
    ('walkerconsultants'),
    ('walkme'),
    ('walnut'),
    ('walser'),
    ('wandb'),
    ('warblerlabs'),
    ('waremalcomb'),
    ('wasabi'),
    ('washburn'),
    ('washmen'),
    ('watchguard'),
    ('waterfall'),
    ('waterworks'),
    ('wattpad'),
    ('waveapps'),
    ('wavicledata'),
    ('way'),
    ('wayflyer'),
    ('wc-br'),
    ('wealthfinancialtechnologies'),
    ('wealthfront'),
    ('wealthsimple'),
    ('wearatoms'),
    ('weareaccord'),
    ('wearedream'),
    ('webfx'),
    ('websummit'),
    ('weekdayworks'),
    ('weekender-hotels'),
    ('weel'),
    ('weinfuse'),
    ('wellfit'),
    ('welocalize'),
    ('wemaintain'),
    ('wepay'),
    ('weride'),
    ('wesalute'),
    ('westair'),
    ('whereby'),
    ('whiterabbit'),
    ('whiterivertoyota'),
    ('whoop'),
    ('whymonster'),
    ('wilburlabs'),
    ('wildlight'),
    ('willowinc'),
    ('winamax'),
    ('windfalldata'),
    ('windownation'),
    ('windowsbyrba'),
    ('windsormanagement'),
    ('wingedrecruitment'),
    ('wingtra-2'),
    ('winnco'),
    ('wintermute-trading'),
    ('wisdomtree'),
    ('wisdomtree-2'),
    ('wisecode'),
    ('wish'),
    ('wishpond'),
    ('wisk'),
    ('wisprai'),
    ('wisr'),
    ('with-foundation'),
    ('withclutch'),
    ('withdouble'),
    ('withmerlin'),
    ('wiz'),
    ('wmg'),
    ('wolt'),
    ('wolve'),
    ('wonderbly'),
    ('wonolo'),
    ('woodardcurran'),
    ('work-life'),
    ('workbravely'),
    ('workingfamilies'),
    ('workinprogress'),
    ('workos'),
    ('workramp'),
    ('workwave'),
    ('woven-by-toyota'),
    ('woven-planet'),
    ('wpromote'),
    ('wr'),
    ('wra-ca'),
    ('wristcheck'),
    ('wwprosolutions'),
    ('wyetechllc'),
    ('wyld'),
    ('wyze'),
    ('x1creditcard'),
    ('xage-security'),
    ('xagroup'),
    ('xby2'),
    ('xcimer'),
    ('xepelin'),
    ('xero'),
    ('xfers'),
    ('xilis'),
    ('xmodesocial'),
    ('xometry'),
    ('xsolla'),
    ('xxartists'),
    ('yardstick'),
    ('yardzen'),
    ('yassir'),
    ('yellowai'),
    ('yelp'),
    ('yieldstreet'),
    ('yottaa'),
    ('you'),
    ('younited'),
    ('yourmechanic'),
    ('yubico'),
    ('yubo'),
    ('yuno'),
    ('ywcaworks'),
    ('z1tech'),
    ('zadara'),
    ('zaimler'),
    ('zanskar'),
    ('zapsurgical'),
    ('zartis'),
    ('zayzoon'),
    ('zazzle'),
    ('zededa'),
    ('zeeco'),
    ('zeitgold'),
    ('zeller'),
    ('zencore'),
    ('zendesk'),
    ('zendrive'),
    ('zeneducate'),
    ('zenogroup'),
    ('zensurance'),
    ('zentail'),
    ('zenysis'),
    ('zeotap'),
    ('zeplin'),
    ('zerion'),
    ('zerocater'),
    ('zerofox'),
    ('zerohomes'),
    ('zerotier'),
    ('zesty'),
    ('zeta'),
    ('zeus'),
    ('zignallabs'),
    ('ziing'),
    ('zilliz'),
    ('zimperium'),
    ('zinccollective'),
    ('zingtree'),
    ('zinovo'),
    ('zip'),
    ('zipari'),
    ('zipongo'),
    ('zivaro'),
    ('zocks'),
    ('zoomer'),
    ('zoomprop'),
    ('zoox'),
    ('zopa'),
    ('zotecpartners'),
    ('zowie'),
    ('zoyi'),
    ('zumper'),
    ('zurichinstruments'),
    ('zurigroup'),
    ('zuru'),
    ('zushealth'),
    ('zuva')
) AS t(slug)
ON CONFLICT (board_token) DO NOTHING;

-- Verify
SELECT COUNT(*) AS total_tokens FROM api.lever_board_tokens;
