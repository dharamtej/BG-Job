...
public partial class IcimsJobsJobHandler : IJobHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var tokens   = await fetchDa.GetActiveIcimsTokensAsync(cancellationToken);
            ...
            for (int i = 0; i < tokens.Count; i++)
            {
                ...
                _logger.LogInformation("[iCIMS] [{I}/{T}] {Company} ({Token}) H1B={H1B}",
                ...
                try
                {
                    ...
                    if (status != null)
                        await fetchDa.UpdateIcimsTokenStatusAsync(token.Id, status, code, count, cancellationToken);
